...
<p align="center">
  <img src="./docs/assets/nirvana-promo-en-readme.gif" alt="One prose order in — a whole AI conglomerate of companies, squads, and agents assembles and delivers, with a gate on every step" width="100%">
</p>

![Nirvana-OS — command a universe of companies in plain language](./docs/assets/hero.png)

[![version](https://img.shields.io/badge/version-0.3.3--beta-blue)](#license-authorship-and-status)
...
> **See it in action:** the [documentation home](https://gutomec.github.io/nirvana-os-engine/) runs the same sentence inside all seven runtimes: Claude Code, Codex, Gemini, Antigravity, Grok, Kimi, and Hermes.
...
> **Prefer a visual walkthrough?** The [illustrated install guide](https://gutomec.github.io/nirvana-os-engine/install.html) shows every command and its real terminal output, step by step.
...
The Studio canvas is a visual layer on top of the same protocol everything else uses: describe what must be built in the entry block (documents, transcripts and URLs attach to it), let the planner propose the graph, validate it, and explicitly confirm before `Build` materializes only entities and relationships that have canonical non-interactive lifecycle adapters. Unsupported relations, including source-backed mind-clone creation until Genius Factory exposes its adapter, remain reviewable plans and fail closed rather than being reported as built. State lives in `~/.nirvana/studio/graphs/` globally or `.nirvana/studio/graphs/` per project; node state persists after each result and standard indexers run before a build reports success.
...
Status: beta (0.x, currently 0.1.59). The engine works today and installs in minutes. Expect the surface to keep moving until 1.0.
