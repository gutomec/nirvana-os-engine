...
A lease protege início, execução e retomada. A renovação é responsabilidade da porta, nunca do adapter. O abort encerra o subprocesso dos adapters de dispatch (ver [adapters de dispatch](gauntlet-multi-target-adapters.md)); um adapter que ignore o sinal continua rodando por conta própria, e ainda assim seu resultado é descartado. Ainda não há política de recovery humano; a entrada pública é o comando `nrv multi-target run`.
