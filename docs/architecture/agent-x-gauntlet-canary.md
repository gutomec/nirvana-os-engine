...
A parada é sempre do controller: `success`, `max_rounds`, `max_cost`, `max_duration`, `no_progress`, `critical_regression`, `judge_disagreement` ou `execution_failure`. Só a revisão apontada em `selectedRevisionId` é publicada em `outputsRoot` e passa pelo `runDelivery` existente, que continua sendo o quality gate obrigatório. Qualquer outra parada encerra o Run em `withheld` com a razão no payload da transição. Em qualquer dessas saídas, e também quando o Gauntlet nem começa (avaliador indisponível, exit 4; `max_cost` antes do producer, exit 1), a linha do run-ledger legado sob o mesmo `run_id` termina coerente com o Run ([mapa canônico → legado](run-kernel-operations.md#compatibilidade-e-cutover)), e o `brief-squad`/`brief-business` que o `dispatch.ts` roda para montar o projeto não abre a própria linha agêntica (`NIRVANA_DISPATCH_TRACKS_RUN=1`): nenhuma linha sobrevive ao processo como `running`.

No `dispatch.ts`, os três call sites reutilizam o mesmo producer para o candidate inicial e para cada revisão: mesma função de execução, mesmo runtime, com o brief original mais a seção "Defeitos a corrigir" gravada ao lado das revisões do candidate. O Business reconstrói o prompt do employee por candidate, para que cada revisão escreva no próprio diretório. O custo reservado por rodada é a estimativa por candidate multiplicada por `candidateStrategy.count`, sem ultrapassar o teto do plano; a estimativa divide o teto pelo número de rounds e de candidates, e `--max-budget` só a reduz. Com avaliador real, a avaliação toma 25% dessa parcela ou o piso de USD 1,50, o que for maior; o produtor fica com o restante, e uma parcela que o piso consome inteira rola o Run para `max_cost` antes do producer.

Os três call sites também compartilham a escolha do avaliador, feita antes do primeiro producer e gravada no audit (`x_gauntlet_evaluator_selected`, `x_gauntlet_evaluator_fallback`): `NIRVANA_GAUNTLET_EVALUATOR`, senão um squad instalado que declare `quality.specification_conformance`, senão o `judge-x` do engine, para qualquer producer. Sem judge disponível o Gauntlet não inicia (`x_gauntlet_evaluator_unavailable`, exit 4, Run `rolled_back` como `evaluator_unavailable`); a heurística do quality gate offline só entra por `NIRVANA_GAUNTLET_EVALUATOR=heuristic`. O avaliador real roda como subprocesso do `dispatch.ts` em `.nirvana/gauntlet/<run>/evaluations/<revisionId>/` e devolve um `scorecard.json` validado contra o contrato do plano; as `revisionRequests` desse scorecard são o que a seção "Defeitos a corrigir" entrega ao producer. Detalhes em [Contrato do avaliador do Gauntlet](gauntlet-evaluator-contract.md).
...
- Nenhum squad instalado declara `quality.specification_conformance` hoje, então sem `NIRVANA_GAUNTLET_EVALUATOR` o avaliador é o `judge-x` do engine para os três producers, no runtime do dispatch; um runtime sem persona do judge (`qwen-code`, `opencode`) não abre Gauntlet. Uma avaliação `indeterminate` (scorecard ausente, inválido, fora do contrato, ou cota do juiz esgotada) encerra o Run `withheld` sem revisão e sem gate final. Na heurística, disponível só por opt-in explícito, a nota é a fração de arquivos avaliáveis aprovados, e um candidate com nota zero em `light` para em `no_progress` já na primeira rodada, porque a paciência do perfil é uma rodada.
