...
- Nenhum squad instalado declara `quality.specification_conformance` hoje, então sem `NIRVANA_GAUNTLET_EVALUATOR` o avaliador é `agent-x` para producers Squad e Business e a heurística do quality gate offline para o producer agent-x. Na heurística a nota é a fração de arquivos avaliáveis aprovados, e um candidate com nota zero em `light` para em `no_progress` já na primeira rodada, porque a paciência do perfil é uma rodada. Uma avaliação `indeterminate` (scorecard ausente, inválido ou fora do contrato) encerra o Run `withheld` sem revisão e sem gate final.
