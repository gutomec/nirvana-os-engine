...
| Arquivo | Conteúdo |
|---|---|
| `evaluation-brief.md` | O brief de avaliação, em PT-BR como todo brief que o engine entrega a um executor: identificação do Run, do candidate e da revisão; a regra de não produzir nem editar; o caminho somente leitura do candidate; a nota de que shell/execução de comandos está disponível para OBSERVAR o candidate (rodar os testes dele, medir comportamento real, usar um browser quando o ambiente tiver um) — nunca para criar, alterar ou produzir algo nele, e que uma afirmação de comportamento não é evidência até o próprio avaliador rodar ou ver o que ela alega; o brief original; o contrato de sucesso em tabela (id, capability, bloqueante, nota mínima, descrição); o caminho do scorecard; o formato JSON com as regras; a guarda de escopo; e, quando o plano marca holdout `evaluator_only`, a instrução de não compartilhar critérios com o produtor. |
...
**Persona.** Sete arquivos, um por runtime, `skills/_shared/agents/judge-x.<runtime>.md` (claude-code, codex, gemini, antigravity, grok, kimi, pi), na mesma estrutura das personas do agent-x e cobertos pelo gate `check-scope-guard`. A persona é curta e fechada: lê o brief de avaliação, o contrato e o candidate, pode rodar os testes do próprio candidate e usar um browser quando o ambiente tiver um (para observar o comportamento real, nunca para produzir ou editar), escreve um único `scorecard.json` no `output_path`, evidência por arquivo, trecho ou execução observada, nota conservadora, `indeterminate` quando não consegue julgar, e a guarda de escopo. Sem recrutamento de squads, sem `nrv`, sem rollover. A resolução é estrita: `judge-x.<flavor>.md` do runtime, nunca a persona de outro runtime; um runtime sem persona (`qwen-code`, `opencode`) não tem juiz.
