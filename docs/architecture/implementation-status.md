...
| Gauntlet Engine | Engine implementada, com canário operacional | Compiler, stores, controller, budgets, replay, `agent-x light`, Squad único `light`, política multi-target e reserva agregada determinística | Política e reserva multi-target ainda não estão conectadas ao executor; múltiplos targets, `balanced` e `exhaustive` ainda não usam o controller |
