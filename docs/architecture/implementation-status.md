...
| Gauntlet Engine | Engine implementada, com canário operacional | Compiler, stores, controller, budgets, replay, `agent-x light`, Squad único `light` e política multi-target determinística | A política multi-target ainda não está conectada ao executor; múltiplos targets, `balanced` e `exhaustive` ainda não usam o controller |
