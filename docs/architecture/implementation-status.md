...
| Gauntlet Engine | Engine implementada, com canário operacional | Compiler, stores, controller, budgets, replay, `agent-x light`, Squad único `light`, política, reserva e coordenador multi-target injetável | O coordenador multi-target ainda não tem adapters de produção nem cutover público; `balanced` e `exhaustive` ainda não usam o controller |
