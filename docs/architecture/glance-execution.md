...
O fluxo abaixo é o de `mode: "run"`. O padrão do chat canônico é o turno do maestro, descrito em [O turno do maestro](#o-turno-do-maestro).

1. `POST /api/v1/conversations/:cnv/messages` com `mode: "run"` persiste a Message, prepara o Run canônico com `policySnapshotRef: gauntlet-light-canary` e responde `202` quando o Run entrou na fila. O recibo é imediato: nunca espera o roteador.
...
Uma Message do chat canônico não prepara um Run: é um turno do runtime do projeto (`POST /api/v1/conversations/{cnv}/messages` com `mode: "turn"`, o padrão; `mode: "run"` mantém o fluxo acima para clientes de API). O módulo é `skills/harness/lib/control-plane/maestro-turn.ts`, o mesmo que o chat legado usa (`/api/actions/chat-agent` → `chat-concierge.ts`, hoje um invólucro fino).

1. A Message é persistida como `user`; o servidor responde `202` com `turn` (`turn_id`, `state` `queued` ou `running`, `position`, `runtime`, `session_id`), `session` (o índice da conversa, com `resume_command`) e `events_url`. Sem `glance.execution`, ou sem o runtime no PATH, responde `200` com `turn.state: "unavailable"` e `reason: capability_unavailable`, sem iniciar nada; `--read-only` recusa a escrita como toda escrita do `/api/v1`.
2. A fila da conversa inicia o runtime headless na raiz do projeto, líder do próprio grupo de processos, com a Message no stdin: `claude -p --output-format stream-json --include-partial-messages --verbose --session-id <uuid novo> | --resume <session_id> --append-system-prompt "<diretiva do maestro>" [--model …] --dangerously-skip-permissions [--max-budget-usd <glance.maestro_max_budget_usd>]`. No Windows o `claude.cmd` roda pelo interpretador de comandos, que não carrega uma quebra de linha num argumento; ali a diretiva vai como `--append-system-prompt-file <arquivo temporário>`, removido quando o filho termina. Os outros runtimes passam pelo `runHeadless` do driver (`codex exec resume <sid>`, `gemini -r <sid>`, flags de autonomia por runtime) num filho do módulo. O env do filho é o do runner de execução: settings efetivas fixadas (`settingsEnvForChild`), `NIRVANA_PROJECT_ROOT` e `HARNESS_LOGS_DIR` no log do projeto. O runtime é o de `detectExecutionRuntime` (host da sessão, `execution.default_runtime`, PATH); a autonomia segue `execution.headless_skip_permissions`.
3. A saída é normalizada (`{t:"tok"}` tokens; `{t:"tool", name, cmd}` chamadas de ferramenta; `{t:"run", run_id, trace_id, target_kind, target_slug}` quando o audit do projeto registra `x_ledger_run_opened` durante o turno; `{t:"done", state, result, session_id, resume_command, cost_usd}`) e servida por SSE em `GET /api/v1/conversations/{cnv}/turns/{trn}/events` (`id: <sequence>`, `Last-Event-ID` retoma; o stream fecha depois de `done`).
4. No fim, a resposta é gravada uma vez em `conversation_messages` como `assistant` (`result_message_id` no turno); a conversa guarda `session_id`, `session_runtime`, `session_started_at` e `last_turn_at`; o custo (`total_cost_usd` do `result`) vai ao audit do projeto como `cost_emission` (`source: glance-maestro-turn`, `trace_id` = `turn_id`) e à bolha; o cabeçalho mostra o id curto da sessão e o comando para continuar no terminal.
5. Um turno por conversa por vez: a segunda Message entra na fila (`state: queued`, `position`) e roda quando a primeira termina, retomando a sessão que ela deixou. `POST …/turns/{trn}:cancel` manda `SIGTERM` ao grupo de processos e o turno termina `cancelled`. `GET /api/v1/conversations/{cnv}` traz `session` e `active_turn`, então um reload da página reassina o turno em andamento.

Trabalho despachado pelo maestro (`brief-*`, `nrv dispatch --exec`) abre Runs pelo caminho normal: aparecem na aba Runs e na timeline como hoje, e a bolha mostra o link assim que o Run existe. A diretiva do maestro é um sufixo curto de system prompt em PT-BR (`MAESTRO_DIRECTIVE`), nunca o `SKILL.md` do harness; o catálogo entra como uma linha de slugs, e o maestro consulta `nrv find` para o resto. O modelo de sessão (sessão nativa por diretório, índice, recapitulação quando o runtime apagou a sessão, limites por runtime) está em [Sessões do maestro](maestro-sessions.md). Prova: `glance-maestro-turn.test.ts`, com um `claude` falso que fala stream-json.
...
| Variável | Efeito |
|---|---|
| `NIRVANA_GLANCE_EXECUTION=0` | Sobe o cockpit sem runner; Messages terminam em `capability_unavailable`, e os turnos do maestro respondem `unavailable` com a mesma razão. `--read-only` também desliga a execução. |
...
| `NIRVANA_PROJECT_ROOT` | Root do projeto servido; o runner o repassa ao filho, e o dispatch abre `<root>/.nirvana/run-kernel.sqlite`. |
