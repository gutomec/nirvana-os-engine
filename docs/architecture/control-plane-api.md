...
CLI, Glance e `nrv serve` são adapters dos mesmos application services. O que um agente despachado pelo `nrv serve` alcança do ambiente e dos arquivos do servidor, e o que nunca sai numa resposta, está em [Operação do `nrv serve` com segredos](serve-hardening.md). Uma execução em curso é encerrada com `DELETE /v1/jobs/<trace>`, que manda SIGTERM e devolve o envelope no estado `cancelled` com `signalled` dizendo se um processo foi de fato alcançado; cancelar execução terminada é no-op. Base HTTP proposta: `/api/v1`. Toda escrita exige `Idempotency-Key`; atualização concorrente usa `If-Match`; erro usa `application/problem+json` e inclui `correlation_id`.
...
| `GET /conversations/{id}` | Conversation com `messages`, `session` (`session_id`, `session_runtime`, `resume_command`) e `active_turn` |
| `POST /conversations/{id}/messages` | Message e, por padrão (`mode: "turn"`), um turno do maestro (`202` com `turn`, `session` e `events_url`; `200` com `turn.state: "unavailable"` sem execução); com `mode: "run"`, o eventual Run (`202` quando o Run entrou na fila de execução) |
| `GET /conversations/{id}/turns/{turn}` | Turno do maestro (`state`, `session_id`, `cost_usd`, `result_message_id`, `runs`) |
| `GET /conversations/{id}/turns/{turn}/events` | SSE do turno (`tok`, `tool`, `run`, `done`), retomável por `Last-Event-ID` |
| `POST /conversations/{id}/turns/{turn}:cancel` | `SIGTERM` no grupo de processos do turno; `202` com `cancelling` ou `cancelled` |
| `POST /conversations/{id}:fork` | Conversation com ancestry |
| `GET /runs/{id}` | Run projection, com `route` (origem e razão do alvo); uma Message roteada pela fila recebe `target` e `route` em `x_run_route_resolved`, e não tem `route` enquanto o roteador decide |
...
Por padrão (`mode: "turn"`) uma Message de projeto adotado é um turno do maestro: o runtime do projeto roda headless na raiz do projeto com a sessão da conversa (`session_id` e `session_runtime` persistidos em `conversations`), os eventos chegam pelo SSE do turno, a resposta é gravada uma vez como `assistant` e o custo vai ao audit como `cost_emission`; uma segunda Message da mesma conversa espera na fila. Detalhes em [O turno do maestro](glance-execution.md#o-turno-do-maestro) e [Sessões do maestro](maestro-sessions.md). Com `mode: "run"`, vale o que segue.

Uma Message de projeto adotado prepara um Run com `policySnapshotRef: gauntlet-light-canary` e o entrega à fila do Glance; o recibo `202` é imediato e nunca espera o roteador. O texto pode nomear o alvo no início: `use business <slug>:` ou `use squad <slug>:`, e então o Run nasce tipado, com `route: { source: "explicit", rationale }` no payload de `run.prepared`, na projeção de `GET /runs/{id}` e no recibo. Sem isso o Run nasce em `agent-x` e sem `route`, e a fila consulta o roteador agêntico antes de iniciar o filho: o alvo é a empresa que ele aponta, senão o squad único que ele aponta, senão `agent-x`; a decisão entra no journal como `x_run_route_resolved` com `target` e `route: { source: "router" | "fallback", rationale }`, e `GET /runs/{id}` passa a mostrá-los. Quando o roteador devolve `no_match`, o Run termina `rolled_back` (`reason: no_dispatchable_target`) sem executar, e a conversa recebe uma mensagem `assistant` (com `run_id`) com a razão do roteador e como pedir trabalho ou nomear o alvo; quando o roteador falha e `routing.on_router_failure=fail`, o Run termina `rolled_back` (`reason: router_failed`), também depois do recibo. `POST /runs/{id}:cancel` durante a resolução interrompe o roteador e reverte o Run (`cancelled_before_execution`). Regra completa em [Roteamento da Message](glance-execution.md#roteamento-da-message). Com runner configurado, o Run roda em um processo filho do `dispatch.ts` com `--run-id`, e a timeline (`glance.child_started`, eventos do Gauntlet, `glance.child_exited`) chega pelo stream. `POST /runs/{id}:cancel` mata o filho e conclui `cancelling → cancelled`. Detalhes, recuperação após restart e variáveis de ambiente em [Execução no Glance](glance-execution.md).
