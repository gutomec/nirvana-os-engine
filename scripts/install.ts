...
const RUNTIME_SKILL_DIRS = [
  ...
  join(HOME, ".pi/agent/skills"),   // Pi Coding Agent (Agent Skills standard; only linked if ~/.pi/agent exists)
];
...
function cleanRuntimeEntry(linkPath: string): void {
  ...
  if (existsSync(join(linkPath, COPY_MARKER))) { rmSync(linkPath, { recursive: true, force: true }); return; } // our copy
  const bak = `${linkPath}.pre-nirvana.bak`;
  if (!existsSync(bak)) renameSync(linkPath, bak);   // real user dir → backup once
