...
const RUNTIME_SKILL_DIRS = [
  ...
  join(HOME, ".pi/agent/skills"),   // Pi Coding Agent (padrão Agent Skills; só linka se ~/.pi/agent existir)
];
...
// Força COPIAR as skills para os dirs de runtime em vez de symlink (Windows já
// força isto automaticamente; útil em qualquer FS/runtime que não resolve link).
...
// Point every installed runtime at the ONE shared skills tree. Symlinks são
// usados onde funcionam de verdade (Mac/Linux). No WINDOWS — e para o CODEX, que
// não resolvem symlink de skills de forma confiável — COPIAMOS a árvore (sem
// node_modules; a execução continua resolvendo de ~/.nirvana/skills via
// paths.CLAUDE_SKILLS_DIR). Uma pre-existing REAL dir do usuário é preservada
// (backup .pre-nirvana.bak), nunca destruída; cópias nossas anteriores levam um
// marcador e são regeneradas idempotentemente (sem acumular backups).
const COPY_MARKER = ".nirvana-skill-copy";

function cleanRuntimeEntry(linkPath: string): void {
  if (isSymlink(linkPath)) { rmSync(linkPath, { force: true }); return; }
  if (!existsSync(linkPath)) return;
  if (existsSync(join(linkPath, COPY_MARKER))) { rmSync(linkPath, { recursive: true, force: true }); return; } // cópia nossa
  const bak = `${linkPath}.pre-nirvana.bak`;
  if (!existsSync(bak)) renameSync(linkPath, bak);   // dir real do usuário → backup uma vez
  else rmSync(linkPath, { recursive: true, force: true });
}

function copyRuntimeSkill(target: string, linkPath: string): boolean {
  try {
    // dereference:false + filtro: não arrasta node_modules na cópia (pesado).
    cpSync(target, linkPath, { recursive: true, dereference: false, filter: (s) => basename(s) !== "node_modules" });
    // deps: junction node_modules → deps compartilhadas, para que scripts invocados
    // a partir do dir copiado (ex.: `bun ~/.claude/skills/<s>/scripts/x.ts`) também
    // resolvam `require('yaml')`/`import 'zod'`. Junction é transparente p/ o resolver.
    try {
    ...
    } catch { /* best-effort — execução canônica via ~/.nirvana ainda resolve */ }
    try { writeFileSync(join(linkPath, COPY_MARKER), "Cópia de ~/.nirvana/skills regenerada por `nrv install`. Edite a fonte, não isto.\n"); } catch {}
    return true;
  } catch (e) {
    // Diz o PORQUÊ: um "fail" mudo obriga o usuário a depurar às cegas.
...
/**
 * Bibliotecas de conteúdo do usuário, criadas VAZIAS.
 *
 * O engine é core-only (não embarca squads/empresas/clones), mas o usuário
 * precisa dos diretórios existindo para criar os dele — e o `nrv doctor`
 * checa a existência deles. O escopo de projeto já faz isso
 * (`init-project.ts` cria `.nirvana/{squads,businesses,mind-clones}`); aqui é
 * o equivalente global.
 *
 * NÃO É DESTRUTIVO: `mkdir` recursivo é no-op quando o diretório já existe —
 * nada é apagado nem sobrescrito, e rodar o instalador de novo é idempotente.
 * Cross-OS: só `path.join` + `mkdirSync` (sem comando de shell), com EEXIST
 * tolerado porque no Windows o Bun o lança mesmo com `recursive: true`.
 */
...
function linkRuntimes(): void {
  ...
  for (const rtDir of RUNTIME_SKILL_DIRS) {
    ...
    // Copia quando: Windows, --copy-skills, ou Codex (basename do parent === .codex).
    ...
    for (const s of SKILLS) {
      ...
      try {
        cleanRuntimeEntry(linkPath);
        if (preferCopy) {
          mode = copyRuntimeSkill(target, linkPath) ? "copy" : "fail";
        } else {
          // Symlink de DIRETÓRIO: no Windows usa junction (não pede Developer
          // Mode nem admin); em symlink comum sem privilégio cai no copy.
...
/** Mudança de contrato detectada entre o instalado e o que está chegando. */
interface BreakingChange { slug: string; detail: string; migration?: string }

/**
 * Delega ao helper compartilhado para que este instalador e o install-content
 * relatem exatamente a mesma coisa. A lógica NÃO pode ser reescrita aqui: os dois
 * já duplicam `syncKind`, e foi essa duplicação que quase deixou o caminho de
 * atualização do comprador sem aviso de quebra.
 */
function contractBreaks(installedDir: string, incomingDir: string, label: string): BreakingChange[] {
  try {
    // O repo é a fonte durante a instalação; ~/.nirvana cobre execuções a partir
    // da árvore já instalada.
...
function syncKind(kind: string, srcRoot: string, dstRoot: string, available: string[], old: Record<string, string>, dry: boolean): SyncResult {
  ...
  for (const slug of available) {
    ...
    if (!existsSync(dst)) {
      ...
      // Colisão de slug: existe em disco mas o pack NUNCA o possuiu (não está no
      // manifesto) — ou seja, é criação do usuário com o mesmo nome de um
      // componente nosso. O pack é a fonte da verdade e vence, por política;
      // mas sobrescrever em silêncio faz o trabalho dele sumir sem explicação.
      // Avisar não é backup nem merge: só torna a perda visível e acionável.
      ...
      if (prev !== h) {
        ...
        // ANTES do mirror: é o único momento em que a superfície instalada e a
        // que está chegando coexistem. Depois de sobrescrever, o contrato antigo
        // deixou de existir e a comparação seria impossível.
...
async function offerStarterPack(): Promise<void> {
  ...
  // Quebras de contrato ganham bloco próprio: um "~ atualizado" não distingue
  // melhoria de prosa de uma capability que sumiu, e é a segunda que faz o
  // projeto do usuário parar de funcionar sem ninguém perceber.
  ...
  // Colisões ganham um bloco próprio no fim: numa lista longa de "+/~" a linha
  // que importa passa batida, e esta é a única que significa perda de trabalho.
...
async function main(): Promise<void> {
  ...
  wireHooks();
  linkRuntimes();
  // Antes do starter pack: com --no-starter as bibliotecas vazias precisam
  // existir do mesmo jeito, e com pack elas já estão prontas para receber.
