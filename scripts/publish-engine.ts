...
const EXCLUDES = [
  ...
  // Pack content + catalog/build tooling moved to the private nirvana-packs
  // repo. The dir excludes stay as a defensive guard in case a local checkout
  // ever regenerates them here.
  "starter-pack", "packs-content", ".readme-work", "tmp", "dist", ".DS_Store", "*.bak.*",
  "docs/plans", "docs/nirvana-evolution",
  "docs/ARQUITETURA-E-REPOS.md", "docs/IDENTIDADE-E-RECONHECIMENTO-NIRVANA-OS.md", "docs/OPERACAO.md",
];
...
// .env gate: the owner's personal .env is NEVER publishable. The only one
// allowed is the generic project-skeleton template, with nothing secret-looking.
...
// ── Version badge rewrite ──────────────────────────────────────────────
// The README version badges rot when releases bump (a hardcoded "0.1.57-beta"
// survives into 0.1.58+). Rewrite them in the STAGE from package.json so the
// published repo always shows the real version — source files stay untouched.
{
  const pkgVersion = JSON.parse(readFileSync(join(OUT, "package.json"), "utf8")).version as string;
  const badgeVersion = pkgVersion.replace(/-/g, "--");
  for (const e of readdirSync(OUT, { withFileTypes: true })) {
    if (!e.isFile() || !/^README(\..+)?\.md$/.test(e.name)) continue;
    const p = join(OUT, e.name);
    const src = readFileSync(p, "utf8");
    const out = src.replace(/badge\/version-[^)-]+(--beta)?-blue/g, `badge/version-${badgeVersion}--beta-blue`);
    if (out !== src) writeFileSync(p, out);
  }
  console.log(`  version badge: rewritten to ${pkgVersion}`);
}

// ── AGENTS.md single-source gate ───────────────────────────────────────
// AGENTS.md is the source; CLAUDE.md and GEMINI.md must be byte-identical
// copies. Drift here means the runtimes read different contracts — fail closed.
const agentDocDrift: string[] = [];
{
  const canonical = readFileSync(join(OUT, "AGENTS.md"), "utf8");
  for (const copy of ["CLAUDE.md", "GEMINI.md"]) {
    const p = join(OUT, copy);
    if (!existsSync(p) || readFileSync(p, "utf8") !== canonical) agentDocDrift.push(copy);
  }
}
...
console.log(`  agent docs:    ${agentDocDrift.length === 0 ? "in sync" : `${agentDocDrift.join(", ")} drifted from AGENTS.md — ERROR`}`);

if (watermarks.length || content.length || internalDocs.length || envLeaks.length || agentDocDrift.length) {
  ...
  for (const d of agentDocDrift) console.error(`  agent doc: ${d} != AGENTS.md — run: bun scripts/sync-agent-docs.ts`);
