...
// Gate de .env: NENHUM .env pode shipar, EXCETO o template genérico do
// project-skeleton — e mesmo esse não pode conter nada com cara de segredo.
// O .env pessoal do dono nunca é publicável (regra do dono, 2026-07-02).
...
// pack — tar com cwd + path relativo: um path absoluto do Windows (C:\...) tem
// ":" e o GNU tar do Git Bash o trata como host remoto. cwd = STAGE dispensa o -C.
rmSync(TARBALL, { force: true });
const relTarballRaw = relative(STAGE, TARBALL);
const relTarball = (relTarballRaw === "" || relTarballRaw.includes(":") ? TARBALL : relTarballRaw).split(sep).join("/");
// COPYFILE_DISABLE: sem isso o bsdtar do macOS embute AppleDouble (._*) de
// xattrs no tarball — vira lixo extraído no Linux/Windows (e um ._*.test.ts
// quebra o bun test do comprador).
if (spawnSync("tar", ["-czf", relTarball, "."], { stdio: "inherit", cwd: STAGE, env: { ...process.env, COPYFILE_DISABLE: "1" } }).status !== 0) {
