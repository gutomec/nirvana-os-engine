 ...
 *   skills/{harness,businesses,squads,_shared} + VERSION + EDITION
 *   bin/{nrv,nrv-gemini,nrv-hermes}
 *   scripts/install.ts
 *   package.json (engine deps + version)  +  bun.lock
 *
 * Invariants (build fails otherwise): 4 skills, ZERO deliverable content
...
writeFileSync(join(STAGE, "skills", "EDITION"), "Nirvana-OS Free Edition\n");
...
console.log(`  skills:       ${skillCount}/4`);
...
if (skillCount !== 4 || leaks.length > 0 || wms.length > 0) {
