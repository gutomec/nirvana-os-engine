...
// NEUTRAL engine label — never "Free". Whoever installs a paid pack overwrites
// this file with the product name (see packaging/pack/setup.ts).
...
// pack — tar with cwd + relative path: an absolute Windows path (C:\...) has
// ":" and Git Bash's GNU tar treats it as a remote host. cwd = STAGE spares the -C.
...
// COPYFILE_DISABLE: without it macOS bsdtar embeds AppleDouble (._*) files from
// xattrs in the tarball — extracted junk on Linux/Windows (and a ._*.test.ts
// breaks the buyer's bun test).
