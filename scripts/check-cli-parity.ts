...
// INSIDE the top-level dispatch block only.
//
// The scan used to run over the whole file, so any 2-space-indented line
// containing a `)` counted as a subcommand — a comment with a parenthesis, or a
// `case` inside a helper function, invented commands that were never dispatched
// and failed the build. Anchoring on the column-0 `case "$cmd" in … esac` (a
// helper's own case is always indented) means the file can grow functions
// without the gate hallucinating commands.
function bashNames(): Set<string> {
  const src = readFileSync(join(ROOT, "bin", "nrv"), "utf8");
  const start = src.search(/^case\s+"\$cmd"\s+in\s*$/m);
  if (start === -1) throw new Error('bin/nrv: top-level `case "$cmd" in` not found');
  const rest = src.slice(start);
  const end = rest.search(/^esac\s*$/m);
