...
async function ensureBun() {
  ...
  if (IS_WIN) {
    console.log('Instale o Bun e rode de novo:\n  powershell -c "irm bun.sh/install.ps1 | iex"\n');
    process.exit(1);
  }
  const ok = await ask("Instalar o Bun agora (https://bun.sh)?");
  if (!ok) {
    console.log("\nInstale o Bun e rode de novo:\n  curl -fsSL https://bun.sh/install | bash\n");
    process.exit(1);
  }
  if (spawnSync("bash", ["-c", "curl -fsSL https://bun.sh/install | bash"], { stdio: "inherit" }).status !== 0) {
    console.log("\nFalha ao instalar o Bun. Instale manualmente: https://bun.sh");
    process.exit(1);
  }
  bun = findBun();
  if (!bun) {
    console.log("\nBun instalado. Abra um NOVO terminal (pro PATH atualizar) e rode de novo:\n  npx @nirvana-os/cli\n");
    process.exit(0);
  }
  return bun;
}
...
function extract(tarball) {
  ...
  const r = spawnSync("tar", ["-xzf", tarball, "-C", dir], { stdio: "inherit" });
  if (r.status !== 0) {
