...
O teto não era teórico. Medido na biblioteca instalada: **34 de 664 capabilities** com workflow resolvido já carregam componentes acima dele, a maior em 355% (`mi-dashboard-studio`, 232.969 bytes) — ou seja, a cada despacho essas squads entregavam ao agente cerca de um terço da persona e chamavam aquilo de entrega.

O teto também nunca foi uma restrição técnica. Ele limita uma única seção do prompt (o markdown bruto de agente/task), não o prompt inteiro: manifesto, bloco de capability, tabela de workflow, injeção de clone e brief já não têm teto. 65.536 é um padrão configurável (faixa de segurança `[8_192, 1_048_576]`, sobrescrevível via `NIRVANA_LIMIT_SQUAD_PROMPT_COMPONENTS_BYTES_MAX`, `.nirvana-limits.yaml` ou `~/.claude/nirvana-limits.yaml`), introduzido em 27/08/2026 junto com o leitor de workflow v6 — não derivado de nenhuma janela de contexto de modelo ou limite de transporte. O único limiar real de transporte fica em outro lugar e já é tratado: `MAX_ARGV_PROMPT_BYTES` (100.000) no `host-agent-driver.ts`, acima do qual cada runtime troca para o seu canal sem perda — stdin no claude-code, codex, gemini-cli e qwen-code; `--prompt-file` nativo no grok-cli; arquivo anexado no pi; bootstrap em arquivo no agy, kimi e opencode. Nenhum adapter põe prompt sem teto no argv.
...
A nota é medida sobre as duas seções renderizadas, não sobre um contador compartilhado entre elas. Um contador corrido passado da chamada de agentes para a de tasks reportava a metade que cruzasse a linha primeiro e silenciava a outra: na mesma biblioteca, 8 dessas 34 capabilities recebiam um número errado, no pior caso reportando 50.712 bytes de excesso contra 162.692 reais. Nada se perdia de nenhum dos dois jeitos — mas um diagnóstico que subnotifica em 3x continua sendo um diagnóstico que mente.
