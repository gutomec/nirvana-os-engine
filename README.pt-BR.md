...
<p align="center">
  <img src="./docs/assets/nirvana-promo-en-readme.gif" alt="Uma ordem em prosa — e um conglomerado de IA de empresas, squads e agentes se monta e entrega, com um gate em cada passo" width="100%">
</p>

[![version](https://img.shields.io/badge/version-0.3.3--beta-blue)](#licença-autoria-e-status)
...
> **Veja funcionando:** a [home da documentação](https://gutomec.github.io/nirvana-os-engine/) roda a mesma frase dentro dos sete runtimes: Claude Code, Codex, Gemini, Antigravity, Grok, Kimi e Hermes.
...
> **Prefere um passo a passo visual?** O [guia de instalação ilustrado](https://gutomec.github.io/nirvana-os-engine/install.html) mostra cada comando e a saída real do terminal, passo a passo.
...
| `nrv studio` | Construtor em canvas infinito (estilo ComfyUI) — projete empresas, squads e mind-clones visualmente |

O canvas do Studio é uma camada visual sobre o mesmo protocolo que todo o resto usa: descreva o que deve ser construído no bloco de entrada (documentos, transcrições e URLs se anexam a ele), deixe o planejador propor o grafo, valide e confirme explicitamente antes de o `Build` materializar somente entidades e relações que tenham adaptadores canônicos de lifecycle não interativos. Relações sem adaptador, inclusive criação de mind-clones baseada em fontes enquanto a Genius Factory não o expõe, ficam como planos revisáveis e falham de modo seguro em vez de serem informadas como construídas. O estado vive em `~/.nirvana/studio/graphs/` no modo global ou `.nirvana/studio/graphs/` por projeto; o estado de cada nó persiste após o resultado e os indexadores canônicos rodam antes de o build informar sucesso.
...
Status: beta (0.x, atualmente 0.1.59). O engine funciona hoje e instala em minutos. Espere a superfície continuar mudando até o 1.0.
