...
3. **`config.yaml`** local (`~/.nirvana/skills/harness/config.yaml`)
...
Path: `~/.nirvana/skills/harness/config.yaml`. Estado atual após MAX POWER MODE:
...
```javascript
const router = require('~/.nirvana/skills/harness/lib/router');
...
1. Edite `~/.nirvana/skills/_shared/validators/validators.{ts,py}` adicionando ao `Runtime` enum
2. Crie `~/.nirvana/skills/_shared/adapters/<runtime>.md`
...
| `NO_MATCH` em tudo | Registries vazios | `bash ~/.nirvana/skills/{businesses,squads}/scripts/index-*.sh` |
...
- **~/.nirvana/skills/businesses/CONFIGURATION.md** — registry consumido em Stage 0 + Stage 2
- **~/.nirvana/skills/squads/CONFIGURATION.md** — registry consumido em Stage 2
