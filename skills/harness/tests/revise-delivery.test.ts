...
function runRevise(files: Record<string, string | Buffer>, opts: { env?: Record<string, string>; runtimeFails?: boolean } = {}): ReviseCase {
  ...
  fs.writeFileSync(fake, [
    "#!/bin/sh",
    "cat > /dev/null",
    `echo call >> ${JSON.stringify(callsFile)}`,
    opts.runtimeFails
      // A runtime that errors AFTER the edits are on disk — the usual shape of
      // a usage/turn limit at the end of a long revision.
      ? `printf '{"type":"result","is_error":true,"result":"usage limit reached","session_id":"sess-fake","total_cost_usd":0}\\n'`
      : `printf '{"type":"result","is_error":false,"result":"ok","session_id":"sess-fake","total_cost_usd":0}\\n'`,
    "exit 0",
  ].join("\n"));
