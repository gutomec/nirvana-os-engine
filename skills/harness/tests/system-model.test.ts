...
import { sanitizeModelId, resolveSystemModel } from "../../_shared/lib/system-model.ts";
...
describe("resolveSystemModel", () => {
  ...
  test("NIRVANA_MODEL tem prioridade máxima", () => {
  ...
  test("settings.json 'model' saneado (o caminho do bug do usuário)", () => {
    const cfg = mkdtempSync(join(tmpdir(), "cc-"));
    writeFileSync(join(cfg, "settings.json"), JSON.stringify({ model: "claude-fable-5[1m]" }));
    process.env.CLAUDE_CONFIG_DIR = cfg;
    expect(resolveSystemModel("claude-code")).toBe("claude-fable-5");
  });
