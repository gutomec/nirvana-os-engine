...
function runDoctor(prepare: (home: string) => void): DoctorCheck | undefined {
  ...
  // The work dir lives INSIDE the fixture home, and the fixture always owns a
  // `.nirvana` marker, so scope resolution stops here. Otherwise it walks up —
  // and on Windows the temp dir sits under the user profile, so it reached the
  // REAL ~/.nirvana and the doctor happily reported on a registry the test had
  // never written. macOS and Linux hid this: their tmpdir is outside the home.
  ...
  try {
    ...
    // PIN the project root instead of deleting it. resolveScope() resolves a
    // project root by walking up from cwd REGARDLESS of scope mode, so a fixture
    // that only hopes the walk stops in the right place is at the mercy of where
    // the OS puts temp dirs — and Windows puts them inside the user profile,
    // where the walk found the real ~/.nirvana and the doctor reported on a
    // registry this test never wrote.
    env.NIRVANA_PROJECT_ROOT = home;
    delete env.BUSINESSES_REGISTRY_PATH;
    delete env.SQUADS_REGISTRY_PATH;
    env.HOME = home;
    env.USERPROFILE = home;   // os.homedir() follows USERPROFILE on Windows, not HOME
