...
Every scripted dispatch registers a run in the **dispatch run-ledger** (SQLite state-db) and heartbeats while the child runtime works. If a run dies without reaching a terminal state (crash, kill, quota), it does not stall silently: `nrv supervisor sweep` finds expired leases and resumes or re-dispatches them, `nrv supervisor status|watch` inspects them, and every `nrv dispatch`/`nrv run` triggers a lazy background sweep on start (`maybeSweep`, <20ms when nothing is pending). The guarantee is behavioral: a brief that entered the system either reaches a terminal state or is picked up again — never forgotten.
