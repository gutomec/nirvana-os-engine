...
**3. Expert requested and nonexistent → offer to create, and create NATIVELY.** Clone creation is engine work: you, agentically — web research of the person's material (named sources, with dates), 5-layer DNA with `^[FONTE]` on every claim, a MANIFEST with a `routing:` block in the new schema per `_shared/MIND_CLONE_ROUTING_CONTRACT.md` (serves/not_for/refuses/delegates_to), reindex in both scopes and the self-retrieval gate (the `one_liner` must retrieve the clone in 1st place via `nrv find-clone`). The full native path is `_shared/MIND_CLONE_CREATION_PIPELINE.md` and depends on NO squad — the engine installs with zero squads, businesses and clones, and still creates a clone end to end. IF the user has the `fabrica-de-genios` squad installed via pack (capability `knowledge_management.mind_clone_generation_pipeline.execute`), it works as an optional heavy pipeline for large raw-material archives — never as a prerequisite. On both paths, ask before creating: a clone is a permanent artifact in the user's library.
...
**Dispatch is SYNCHRONOUS. Always pass `run_in_background: false`.** The subagent tool defaults to background, and a background dispatch returns *"Async agent launched successfully"* — a launch receipt, not work. An orchestrator that takes that receipt as a result has no idea what its target produced, and the only way left to find out is to go scanning the filesystem for files that may or may not be finished. That is not orchestration, it is guessing: it was measured on a real 13-target run where **all 13 dispatches returned launch receipts and zero results**, and the orchestrator spent the run polling `find`/`ls`, prodding agents with follow-up messages, gating files it happened to notice, and closing ledger runs on the strength of a directory listing.

| Target | Command |
|---|---|
| Business | `bun ~/.nirvana/skills/businesses/scripts/brief-business.ts <slug> "<brief>" --project <trace_id>` then `Agent({subagent_type: "general-purpose", run_in_background: false, prompt: buildEmployeePrompt({...})})` |
| Squad | `bun ~/.nirvana/skills/squads/scripts/brief-squad.ts <slug> "<brief>" --project <trace_id>` then `Agent({subagent_type: "general-purpose", run_in_background: false, prompt: "<read squad.yaml + workflow> + enriched brief path + output_path"})`. The `brief-squad.ts` prep step scaffolds the project dir + HANDOFF, **emits `brief_received`/`dispatch_squad` automatically** (runtime-agnostic audit — you don't rely on `nrv audit emit` firing) and **opens the ledger run**, printing the run id you must close in Phase 7. |
| agent-x | `Agent({subagent_type: "general-purpose", run_in_background: false, prompt: "Read ~/.nirvana/skills/_shared/agents/agent-x.<runtime>.md. Enriched brief at <path>. Output to <output_path>. Trace: <trace_id>."})` |

**Parallel means one message with several calls, never several messages.** A wave of independent targets is dispatched by issuing every one of its `Agent(...)` calls **in a single message** — the runtime runs them concurrently and hands you every result together, at which point the wave is genuinely finished and you can gate it. Firing them one message at a time gives you neither concurrency you can prove nor results you can wait for. Serial order is the opposite move for the opposite reason: dispatch, read the returned result, then dispatch the next with what you learned.

Background dispatch is the exception, not the default, and it costs you the return: use it only when the work is genuinely long *and* you have unrelated work to do meanwhile. If you use it, the runtime notifies you when the agent finishes — **wait for that notification. Never poll the filesystem to infer completion.** A file that exists is not a run that finished, and a run you inferred is a delivery you cannot honestly gate.
...
After dispatch, the dispatched entity self-administers until done. You wait for the return — the actual tool result carrying its report, which is what a synchronous dispatch (Phase 4) gives you and a background one does not. You don't interleave work, and you don't go looking on disk for signs of life: if you find yourself running `find`, `ls` or `stat` to work out whether a target is done, the dispatch was wrong, not the target. The entity (enforced by its own agent file): loads memory first (see **Memory levels** below) → `brief-enriched.md` → its `DISPATCH-INSTRUCTION.md` → upstream `_SUMMARY.md`s; decides with professional defaults (records in `## Premissas assumidas` + `x_assumption_made` events); rolls the context window at ~70% (`HANDOFF.json` + `x_session_rollover` + fresh subagent); may recursively recruit; **verifies before declaring done** (files exist non-empty, criteria met, writes `outputs/_SUMMARY.md`, emits `verify_passed`); escalates via `human_notification_required` when truly blocked; emits `x_plan_change_request` if the upfront plan is wrong (never modifies other phases' outputs).
