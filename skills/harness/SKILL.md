...
**Criar entidades do sistema é trabalho do ENGINE, nunca de squad.** Brief que
pede para criar/melhorar um squad, uma business ou um mind-clone roteia para a
skill de lifecycle correspondente (`squads` → pipeline de criação em
`references/02-creation.md`; `businesses` → wizard + gates no SKILL.md dela;
clone → Regra 9), executada agenticamente por você. Não existe mais squad
criador para despachar — o `nirvana-squad-creator` foi absorvido pelo engine em
2026-07-27, e a régua é a mesma dos gates de lá: pesquisa de domínio
obrigatória, verdade-terreno de roteamento (`example_briefs` roteando de volta
em 1º) e passe de otimização antes de declarar pronto.
...
### Rule 9 — Mind-clone: procure por necessidade, crie se faltar, degrade com honestidade

Mind-clone não é invocado como squad — é **injetado** como "atue como". A seleção segue esta ordem, e ela é fechada: nenhum caminho termina em falha dura.

**1. Nomeado no brief vence tudo.** Se o usuário citou o especialista (slug ou nome), use aquele. Não busque, não questione.

**2. Não nomeado → procure pela NECESSIDADE, não pelo nome.** Você raramente sabe de antemão quem está na biblioteca. Sabe do que a tarefa precisa: uma diretora de elenco, um tipógrafo de livro, alguém que entenda de replicação. Consulte o registro por essa necessidade (`nrv find-clone`, ou a busca do registro) e veja quem cobre. Se ninguém cobrir bem, **não force** — clone mal escolhido é pior que nenhum.

**3. Especialista pedido e inexistente → ofereça criar, e crie NATIVAMENTE.** Criação de clone é trabalho do engine: você mesmo, agenticamente — pesquisa web do material da pessoa (fontes nomeadas, com data), DNA de 5 camadas com `^[FONTE]` em cada afirmação, MANIFEST com bloco `routing:` no schema novo seguindo `_shared/MIND_CLONE_ROUTING_CONTRACT.md` (serves/not_for/refuses/delegates_to), reindex nos dois escopos e gate de auto-recuperação (o `one_liner` tem que recuperar o clone em 1º via `nrv find-clone`). O caminho nativo completo está em `_shared/MIND_CLONE_CREATION_PIPELINE.md` e não depende de NENHUM squad — o engine instala sem squads, sem businesses e sem clones, e mesmo assim cria clone de ponta a ponta. SE o usuário tiver o squad `fabrica-de-genios` instalado via pack (capability `knowledge_management.mind_clone_generation_pipeline.execute`), ele serve como pipeline pesado opcional para acervos grandes de material bruto — nunca como pré-requisito. Em ambos os caminhos, pergunte antes de criar: clone é artefato permanente da biblioteca do usuário.

**4. Não criou → atue por conhecimento próprio, dizendo que é isso.** O dispatch NÃO falha mais por clone ausente; ele injeta um bloco declarando a ausência e devolve `degraded[]`. Quando isso acontecer:

- Trabalhe com o que você sabe sobre o método daquela pessoa.
- **Nunca afirme fidelidade de clone que você não carregou.** Diga que atuou por conhecimento geral.
- **Reporte ao usuário**, no final, quais especialistas faltaram — é ele quem decide se vale criar.

O ponto da regra 4 é a diferença entre degradar e mentir. Trabalhar sem o DNA é aceitável; deixar o usuário achar que o DNA estava lá, não.
