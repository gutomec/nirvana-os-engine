...
Grave pelo caminho canônico (`nrv audit emit`) — schema validado, dual-write SQLite+JSONL, campos de entidade normalizados (`business_slug`/`squad_name`). NÃO use `echo` cru: gravar `business`/`squad` bare quebra o loop de aprendizado (o improver lê `business_slug`/`squad_name`).
...
```
`--auto` picks rubrics by extension: `.md/.txt` → correctness + structure-bounds + wiki-lint; `.json` → json-valid; `.yaml/.yml` → yaml-valid; `.png/.jpg` → brief-fidelity; `.html` → **html-valid** (well-formação estrutural offline: tags balanceadas + estrutura). Override with `--rubrics ...`. Each rubric returns `{passed, score, reasoning, fix_list}`; the driver emits `gate_passed` (exit 0) or `gate_failed` (exit 1). Quando NENHUMA rubrica aplicável roda, o status é `INDETERMINATE` e o exit é não-zero (fail-closed — nunca finge sucesso).

**Julgamento visual** (entregáveis web renderizados). `html-valid` checa a boa-formação ESTRUTURAL (tags balanceadas), **não os pixels**. O julgamento VISUAL — a página renderizada bate com o brief? — é seu, maestro: abra o `.html` (ou renderize) e avalie contra o brief antes de emitir `gate_passed`. Não há script de screenshot automático embarcado no engine.
...
**Loop guard (teto duro do loop).** Antes de cada iteração de revisão — e de cada retry do dispatch cascade na Phase 4 — rode `nrv guard tick --project <projectRoot> --action revision --progress <contagem-de-artefatos-ou-hash>`. Ele rehidrata o estado do `loop_guard_state` no HANDOFF e checa os tetos (`max_steps` 12, `max_repeat` 3 assinaturas iguais, `max_flat_steps` 4 sem progresso). Se sair com código ≠ 0 (`🛑 LOOP GUARD`), **pare de iterar, escreva o HANDOFF e suba ao humano — não re-despache.** Passe um `--progress` que mude quando há progresso real (ex.: número de arquivos entregues), senão `max_flat_steps` dispara em 4 iterações.
