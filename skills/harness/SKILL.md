...
**Visual judgment** (any rendered visual deliverable). `html-valid` checks STRUCTURAL well-formedness (balanced tags), **not the pixels**. The VISUAL judgment is yours, maestro: render or open the final composition at the sizes and states required by the brief before emitting `gate_passed`. Contrast is judged from the final composited result, after overlays, gradients, opacity, images, blend modes and effects. If the artifact contains text, render with the intended fonts and verify that every glyph remains fully inside its visual container with safe padding: no clipping, truncation, unintended overflow, overlap or loss of legibility. Source tokens, CSS declarations and uncomposited layers are not proof. There is no automatic screenshot script embedded in the engine, so do not claim a pixel-level check that did not run.
