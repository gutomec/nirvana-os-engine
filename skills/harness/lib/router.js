...
const RUN_ORG_VERBS = [
  ...
  'organize', 'orchestrate', 'orquestrar', 'agency', 'agencia', 'organization',
  'business', 'empresa', 'agencia', 'conglomerate', 'conglomerado',
  'ongoing', 'continuo', 'recorrente', 'mensalmente', 'monthly', 'quarter',
  'trimestre', 'campaign', 'campanha', 'cliente', 'client', 'account',
];
const WORK_VERBS = [
...
function applyAdjustments(results, intent, briefText) {
  ...
  for (const r of results) {
    ...
    const boost = meta.score_boost != null ? meta.score_boost : 1.0;
    score *= boost;
...
const META_INTENT_KEYWORDS = [
  ...
  'landing.*copy', 'copy.*paid', 'landing.*paid', // multi-deliverable patterns
  ...
  // pedido vago tipo "crie uma landing" — vai para amplify
  'crie uma landing', 'criar landing', 'cria landing',
  'crie uma brand', 'crie um brand',
  'preciso de landing', 'preciso de brand', 'preciso de copy', 'preciso de ads',
  'quero landing', 'quero brand', 'quero copy', 'quero ads',
];
...
async function route(brief, ctx) {
  ...
  // Stage 2 — Capability matching (BM25 over both pools)
  const matches = stage2Match(intent, registries, { brief, topK: 10 });
  ...
  if (prefer === 'business' && matches.length > 0) {
    ...
    if (bestBusiness && bestBusiness.normalized >= businessFirstThreshold) {
      // Promote business pool to the front; keep squads as alternatives.
      orderedMatches = [...businesses, ...squads];
