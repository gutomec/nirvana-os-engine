...
function buildMatchDocs(squadsRegistry, businessesRegistry) {
  ...
  if (businessesRegistry && businessesRegistry.businesses) {
    for (const [slug, b] of Object.entries(businessesRegistry.businesses)) {
      ...
      // example_briefs/produces/keywords entram no texto indexado (2026-07-27):
      // sem eles a business era invisível por construção para o PRÓPRIO brief
      // que declarou — medido: 24/319 example_briefs de business chegavam ao
      // destino certo, porque o doc só indexava slug+description+domains+caps
      // enquanto squad capability indexa o brief verbatim. Mesma classe de
      // defeito dos mind-clones sem bloco routing (MRR 0.05).
...
/**
 * Cobertura de conteúdo: quantos tokens de conteúdo do brief o documento casa.
 * Tokens de conteúdo = saída do tokenizer do bm25 (stopwords e verbos de
 * andaime já removidos). Alimenta o gate de NO_MATCH por cobertura no Stage 3.
 */
function countMatchedContentTokens(brief, docText) {
  if (!brief || !docText) return { matched: null, total: null };
  const briefToks = new Set(bm25.tokenize(brief));
  if (briefToks.size === 0) return { matched: null, total: 0 };
  const docToks = new Set(bm25.tokenize(docText));
  let matched = 0;
  for (const t of briefToks) if (docToks.has(t)) matched++;
  return { matched, total: briefToks.size };
}
...
function applyAdjustments(results, intent, briefText) {
  ...
  // Filtro de intent OPT-IN (censo 2026-07-27, n=2.358 example_briefs): com o
  // filtro ligado a acurácia pós-ajustes era 94,1% contra 99,8% do BM25 cru, e
  // TODOS os 133 casos derrubados eram erro dele (131) ou de boost 0 (2) — zero
  // curadoria. Verbos banais ("run", "rodar", "organize") classificavam RUN_ORG
  // e excluíam capabilities por classe: 81 NO_MATCH fabricados + 34 HIGH para
  // business_route alheia. Ligue com NIRVANA_ROUTER_INTENT_FILTER=1 para
  // re-medir; o código segue intacto. Ver ROUTER-INVESTIGACAO.md (analista).
  ...
  for (const r of results) {
    ...
    // Piso 1.0: score_boost declarado como 0 era aceito como multiplicador ×0 e
    // auto-aniquilava a capability (2 casos reais no censo, omnidoc
    // automation.deps.bootstrap). Boost é desempate para cima, nunca veto — veto
    // é not_for/refuses.
...
async function stage2MatchHybrid(intent, registries, opts) {
  const brief = (opts && opts.brief) || '';
  // Cobertura sempre contra o brief ORIGINAL do usuário: a amplificação do
  // Stage -2 transforma 4 tokens em ~180, e qualquer vencedor casa ≥3 dos
  // tokens amplificados — o gate de cobertura do Stage 3 ficaria cego. A base
  // de medição do censo (real ≥3 casados, fora de domínio ≤2) é o brief cru.
  ...
  // Braço denso — OPT-IN, e o motivo está medido.
  //
  // Verdade-terreno disponível de graça: cada capability declara `example_briefs`,
  // e o brief que ela declarou deve rotear para ela. Medido sobre n=41 desses
  // briefs, o top-1 do pipeline:
  //
  //     BM25 sozinho ......... 100%   (41/41 — o alvo certo em 1º SEMPRE)
  //     fusão com o denso .....  29%
  //     fusão sem o denso .....  63%
  //
  // A causa é estrutural, não de calibração. O RRF é rank-only com k=60, e este
  // corpus tem ~1.100 documentos: a distância entre `1/60` e `1/61` é 1,7%, então
  // estar presente numa lista lateral rende quase tanto quanto liderar a lista
  // principal, e um candidato mediano em duas listas supera o campeão de uma.
  // Varredura de pesos (bm25 até 10× contra denso 1×) não recupera — com k=60 a
  // constante domina a diferença de rank.
  //
  // Mesma conclusão que a busca de mind-clones (`clone-search.ts`), medida
  // separadamente e no mesmo dia: o embedder de paráfrase mede proximidade de
  // ASSUNTO, e a pergunta aqui é quem DECLARA a competência.
  //
  // Ligue com NIRVANA_ROUTER_DENSE=1 para re-medir; o código segue intacto.
  let denseScored = null;
  if (process.env.NIRVANA_ROUTER_DENSE === '1') {
    try {
  ...
  // A FUSÃO RRF ESTÁ DESLIGADA POR PADRÃO. O modo `fast` é BM25 puro.
  //
  // Este caminho é o determinístico; o modo padrão do sistema é agêntico, com um
  // agente lendo empresas, squads e clones para escolher. Cabe ao `fast` ser
  // simples e previsível — e a medição diz que BM25 sozinho já é o melhor que
  // temos aqui.
  //
  // Verdade-terreno de graça: cada capability declara `example_briefs`, e o brief
  // que ela declarou deve rotear para ela. Sobre n=41 desses, top-1 do pipeline:
  //
  //     BM25 puro ................ 100%   (41/41, alvo certo em 1º SEMPRE)
  //     RRF com denso + cobertura .  29%
  //     RRF só com cobertura ......  63%
  //
  // A falha é estrutural, não de calibração. O RRF é rank-only com k=60 e este
  // corpus tem ~1.100 documentos: entre `1/60` e `1/61` há 1,7%, então estar
  // presente numa lista lateral rende quase tanto quanto liderar a principal, e um
  // candidato mediano em duas listas supera o campeão de uma. Duas hipóteses de
  // conserto foram testadas e descartadas por medição — pesos (bm25 até 10× contra
  // denso 1×: nada passou de 40%) e mover a curadoria para antes da fusão (piorou
  // para 22%). Com k=60 a constante domina a diferença de rank; não há peso que
  // recupere.
  //
  // As `business_route` não perdem nada: elas estão no corpus do BM25 e competem
  // pelo próprio texto. O que sai é só o boost por rank do achado #4d — e o
  // sequestro que aquele achado descrevia já é impedido no Stage 3, que descarta
  // rota cujo regex declarado não dispara.
  //
  // Ligue com NIRVANA_ROUTER_FUSION=1 para re-medir; o código abaixo segue intacto.
...
function stage3Decide(matches, opts) {
  ...
  // Uma `business_route` declara um regex de ativação. Quando esse regex NÃO casa
  // com o brief, a rota não é candidata — é ruído de BM25: os padrões de um mesmo
  // negócio compartilham quase todos os tokens (`ebook|livro|manual`), então
  // `format-only` e `audit-and-revise` pontuavam 0.94 contra o 1.00 de
  // `write-bestseller` num brief que só o último aceita. Três quase-empates
  // fabricados enchiam a janela de ambiguidade e derrubavam HIGH para AMBIGUOUS,
  // com o roteador abstendo entre alternativas que ele mesmo já havia descartado.
  //
  // Filtra só quando sobra alguém: se nenhuma rota casar, a lista segue intacta e
  // a decisão continua sendo tomada pelo score, como antes.
  // A regra é deliberadamente estreita: só filtra quando o PRÓPRIO LÍDER é uma
  // rota cujo contrato dispara. Aí, e só aí, rotas que demonstravelmente não
  // disparam não são concorrentes.
  //
  // A versão larga (filtrar sempre que a rota não casasse) foi medida e REPROVADA:
  // em brief sem domínio ("quanto é dois mais dois") ela removia o falso
  // concorrente, o líder ficava com folga e o sinal virava HIGH apontando um squad
  // de vídeo — troca de abstenção segura por erro confiante. Um piso de score
  // absoluto compensaria, mas não existe: medido sobre example_briefs reais contra
  // briefs fora de domínio, as distribuições se sobrepõem (real chega a 5.5, fora
  // chega a 12.2). Sem separação, qualquer piso erra de um dos dois lados.
  const brief = opts && typeof opts.brief === 'string' ? opts.brief : null;
  // `(?i)` é flag inline de Python/Go; JS lança nela. Como compilamos com 'i',
  // remover o prefixo preserva a semântica — 9 das 498 rotas usam isso.
  const routeFires = (m) => {
  ...
  // NO_MATCH por cobertura (censo 2026-07-27, ROUTER-INVESTIGACAO.md Parte 2).
  // O score normalizado não separa brief real de brief fora de domínio — o topo
  // é sempre 1.0 por construção. A CONTAGEM de tokens de conteúdo casados
  // separa com banda vazia: brief real casa ≥3 tokens do vencedor (fração
  // ≥0,80), fora de domínio casa ≤2 (fração ≤0,50). O corte é por contagem E
  // fração combinadas, nunca fração sozinha (mataria 63% dos briefs reais no
  // pior caso honesto) e nunca contagem sozinha (um brief curto legítimo de 2
  // tokens — "escreva o ebook" — casa 2/2 = fração 1,0 e não pode ser punido).
  ...
  // Um cluster que resolve para o MESMO destino não é ambiguidade. `escreva o
  // ebook` traz `squad_capability:ebook-maestro-nirvana` em 1º e
  // `business_route:ars-libri:ebook-maestro-nirvana::write-bestseller` em 2º — o
  // mesmo squad, um direto e outro pela empresa. O trabalho cai no mesmo lugar, e
  // perguntar "qual dos dois?" não é escolha que o dono possa fazer melhor que o
  // roteador. Antes disso a consulta abstinha, e era o caso H4a/H4b do relatório
  // de validação externa.
  //
  // O agrupamento é por SQUAD de destino, não por capability: quando o trabalho
  // chega no mesmo squad, é o dispatcher dele que escolhe a capability — essa
  // decisão não pertence a este estágio.
  const destino = (m) => {
    const meta = (m && (m.meta || (m.doc && m.doc.meta))) || {};
  ...
  if (cluster.length >= 2) {
    const destinos = new Set(cluster.map(destino));
...
const META_INTENT_KEYWORDS = [
  // EN — "portfolio", "end to end" e "end-to-end" saíram (censo 2026-07-27):
  // eram gatilho banal ("portfolio" 10× era carteira financeira; "end-to-end"
  // 23× descrevia pipeline comum) e sequestraram 56 briefs com 0 acertos.
...
function stageMinusOneMetaIntentDetect(brief, registries, opts) {
  ...
  if (!providers || providers.length === 0) {
    // SEM fallback por substring: a capability canônica não existe no registro
    // atual, e o fallback antigo ("primeira capability contendo 'orchestrate'")
    // pegava ai.squad.orchestrate do squad-forge — 56 briefs sequestrados com
    // 0 acertos no censo de 2026-07-27. Sem a canônica, o Stage -1 se abstém e
    // o pipeline normal roteia; estritamente melhor que um alvo 0% correto.
    return null;
  }
...
async function route(brief, ctx) {
  ...
  const matches = await stage2MatchHybrid(intent, registries, { brief, topK: 10, businessRouteRanked, coverageBrief: originalBrief });
  ...
  if (prefer === 'business' && matches.length > 0) {
    ...
    // O rival do business no desempate é QUALQUER não-business — inclusive uma
    // business_route. Antes o desempate era cego a rotas: com uma route em 1º
    // (1.000) e um squad fraco abaixo (0.599), o business (0.65) era comparado
    // só contra o squad, vencia, e ia para o topo com normalized < 1.0 —
    // quebrando o invariante top=1.0 do Stage 3 (caso real "Run a Monte
    // Carlo…": saiu NO_MATCH top 0.599 com um candidato 1.000 na lista).
