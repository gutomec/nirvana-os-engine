...
import { withLock } from "../../_shared/lib/file-lock.ts";
...
function save(projectRoot: string, reg: Registry): void {
  ...
  // Atomic write (tmp + rename) so lock-free readers never see partial JSON.
  const tmp = `${f}.${process.pid}.${Math.random().toString(36).slice(2)}.tmp`;
  fs.writeFileSync(tmp, JSON.stringify(reg, null, 2));
  fs.renameSync(tmp, f);
}

/** Read-modify-write under the cross-process file lock. Parallel dispatch
 * waves used to race here (load → save lost-update); the lock serializes
 * them. Fail-open on lock trouble (a live-but-stuck holder past the acquire
 * timeout): losing one accounting update beats failing the dispatch. */
function mutate(projectRoot: string, fn: () => void): void {
  try {
    withLock(fileFor(projectRoot), fn);
  } catch (e) {
    console.error(`[spend-tracker] lock unavailable (${(e as Error)?.message ?? e}) — applying update unlocked.`);
    fn();
  }
}
