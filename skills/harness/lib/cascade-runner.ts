 ...
 * long enough that no later step of the same run retries the dead runtime, short
...
export function runWithCascade(args: CascadeRunArgs): CascadeRunResult {
  ...
  let currentOpts: RunHeadlessOpts = {
    ...
    // A session id is only valid in the runtime that created it: a claude-code
    // `--resume <id>` means nothing to codex. If the cascade picked a runtime
    // different from the one the caller requested, the session it brought is
    // from another CLI — drop it and start cold, instead of trying to resume
    // someone else's conversation.
    sessionId: chosen === args.runtime ? args.sessionId : undefined,
  };
  ...
  while (attempt <= maxHandoffs) {
    ...
    if (verdict.kind === "error") {
      // Generic failure: the cause is the brief, a tool call or the machine —
      // not the runtime being unusable. Rotating would just repeat it elsewhere.
      emitAudit({
        event: "runtime_error",
    ...
    if (verdict.kind === "transient") {
      ...
      // Synchronous by design (runHeadless is sync) — but blocking without
      // spinning the CPU. Bun.sleepSync is available in every supported Bun.
      Bun.sleepSync(sleepMs);
      continue;
    }

    // auth_failed and quota_exhausted both mean this runtime cannot serve the run
    // right now, so both take the same route: cool it down, then hand off. The
    // cooldown is per-runtime rather than per-entry because a sibling entry on the
    // same CLI reuses the same credentials and would fail identically — and it is
    // what stops a later step of the SAME run (router retry, agent-x fallback)
    // from picking the dead runtime again. Auth still emits its own loud event:
    // handing off keeps the work moving, it does not hide the broken credential.
    if (verdict.kind === "auth_failed") {
      markCooldown(args.projectRoot, chosen, AUTH_COOLDOWN_SEC, verdict.hint, "auth");
      emitAudit({
        event: "runtime_auth_failed", project_id: args.projectId ?? null,
        runtime: chosen, hint: verdict.hint,
      }, args.projectRoot);
    } else {
    ...
    currentOpts = {
      ...
      model: chosenModel ?? undefined,
      providerHint: chosenProvider ?? undefined,
      // The handoff prompt was rebuilt precisely because the new runtime cannot
      // see the previous session — carrying the id along would contradict that
      // and make the new CLI try to resume a conversation that is not its own.
      sessionId: undefined,
    };
  }
