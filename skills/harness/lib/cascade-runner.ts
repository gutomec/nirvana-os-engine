...
export function runWithCascade(args: CascadeRunArgs): CascadeRunResult {
  ...
  let currentOpts: RunHeadlessOpts = {
    ...
    // Um session id só vale no runtime que o criou: `--resume <id>` do claude-code
    // não significa nada para o codex. Se o cascade escolheu um runtime diferente
    // do que o chamador pediu, a sessão que ele trouxe é de outro CLI — descarta
    // e começa fria, em vez de tentar retomar conversa alheia.
  ...
  while (attempt <= maxHandoffs) {
    ...
    currentOpts = {
      ...
      model: chosenModel ?? undefined,
      providerHint: chosenProvider ?? undefined,
      // O prompt de handoff já foi reconstruído justamente porque o novo runtime
      // não enxerga a sessão anterior — carregar o id junto contradiria isso e
      // faria o CLI novo tentar retomar uma conversa que não é dele.
      sessionId: undefined,
    };
  }
