...
function parseFrontmatter(rawInput: string): { meta: Record<string, unknown>; body: string } {
  // Normalize CRLF → LF first: on Windows, Git checks out .md rubrics with CRLF,
  // so `---\r\n` never matched the `\n`-anchored regex → zero rubrics loaded →
  // the quality gate silently selected nothing. Strip a BOM too.
