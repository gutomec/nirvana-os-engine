...
export async function agenticRoute(args: AgenticRouteArgs): Promise<AgenticRouteDecision> {
  ...
  const businessesReg = path.join(home, ".businesses-registry.json");
...
\`\`\`
{"primary_business":"<slug>","mandatory_squads":["<slug>",...],"optional_squads":["<slug>",...],"rationale":"OBJETO=<x>, TEMA=<y>. <restante da justificativa>"}
\`\`\`
...
Se não há squad nomeado E nenhum business adequado, retorne \`"primary_business": null\` com mandatory_squads vazio e explique. Não invente slugs — só use os que existem no registry.`;
  ...
  if (!res.ok) {
    ...
    return { primary_business: null, mandatory_squads: [], optional_squads: [], rationale: "", ok: false, cost_usd: res.costUsd, duration_ms: durationMs, error: res.error || res.stderr };
  }
  ...
  if (!jsonStr) {
    ...
    return { primary_business: null, mandatory_squads: [], optional_squads: [], rationale: "", ok: false, cost_usd: res.costUsd, duration_ms: durationMs, error: "router did not return JSON" };
  }
  ...
  try { parsed = JSON.parse(jsonStr); }
    ...
    return { primary_business: null, mandatory_squads: [], optional_squads: [], rationale: "", ok: false, cost_usd: res.costUsd, duration_ms: durationMs, error: "router JSON invalid" };
  }
  ...
  return { primary_business: primary, mandatory_squads: mandatory, optional_squads: optional, rationale, ok: !!primary || mandatory.length > 0, cost_usd: res.costUsd, duration_ms: durationMs };
}
