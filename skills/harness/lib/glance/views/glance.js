...
function glance() {
  return {
    ...
    kinds: [
      ...
      { id: 'mind-clones', label: 'Mind-clones' },
    ...
    async boot() {
      ...
      window.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Escape') {
          ...
          if (this.searchResults.length > 0) { this.searchResults = []; return; }
          ...
          // Then go back from detail to list
          if (this.selected) { this.goBack(); }
        }
      });
    ...
    async fetchSquads()   { try { const r = await api('/api/squads'); this.squads = r.squads; this.counts.squads = r.squads.length; } catch(e) {} },
    async fetchBusinesses(){ try { const r = await api('/api/businesses'); this.businesses = r.businesses; this.counts.businesses = r.businesses.length; } catch(e) {} },
    async fetchProjects() { try { const r = await api(`/api/projects${this.projectQuery('?')}`); this.projects = r.projects; this.counts.projects = r.projects.length; } catch(e) {} },
    ...
    async fetchMindClones(){ try { const r = await api('/api/mind-clones'); this.mindClones = r.mind_clones; this.counts['mind-clones'] = r.mind_clones.length; } catch(e) {} },
