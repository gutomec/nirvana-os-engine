...
function glance() {
  return {
    ...
    projectFilter: (typeof localStorage !== 'undefined' && localStorage.getItem('glance.projectFilter')) || '',  // '' = ainda não decidido; fetchScope escolhe project-first
    ...
    async fetchScope() {
      try {
        this.scope = await api('/api/scope');
        // Project-first por padrão: sem preferência salva, o CONSUMO (Runs/Cost/
        // Memory/Agents/Activity) abre no escopo do PROJETO quando há uma pasta
        // de projeto. As bibliotecas (squads/businesses/clones) seguem globais.
        const saved = (typeof localStorage !== 'undefined') && localStorage.getItem('glance.projectFilter');
        if (!saved) {
          const want = (this.scope && this.scope.projectRoot) ? 'project' : 'all';
          this.setProjectFilter(want, false);  // false = não persistir a escolha automática
        }
      } catch(e) {}
    },
    ...
    setProjectFilter(mode, persist = true) {
      ...
      // persist=false para a escolha AUTOMÁTICA (project-first): não vira
      // preferência gravada, então reavalia por projectRoot a cada sessão.
    ...
    async openChatFromHistory(entry) {
      ...
      if (isRun) {
        // Run de produção: reidrata a timeline a partir do audit trail da trace.
        ...
        // Conversa do concierge: não é um run de produção — sem reidratar/subscribe.
        this.chatMessages = [{ role: 'system', text: `Conversa "${entry.title}".` }];
      }
    },
    ...
    loadRulesDraft() {
      const RT = { CLAUDE_CODE: 'claude-code', CLAUDE: 'claude-code', CODEX: 'codex', GEMINI: 'gemini-cli', GEMINI_CLI: 'gemini-cli', ANTIGRAVITY: 'antigravity-cli', ANTIGRAVITY_CLI: 'antigravity-cli', AGY: 'antigravity-cli', KIMI: 'kimi-cli', KIMI_CLI: 'kimi-cli', GROK: 'grok-cli', GROK_CLI: 'grok-cli', HERMES: 'hermes' };
    ...
    rulesToEnv() {
      const SUFFIX = { 'claude-code': 'CLAUDE_CODE', 'codex': 'CODEX', 'gemini-cli': 'GEMINI', 'antigravity-cli': 'ANTIGRAVITY', 'kimi-cli': 'KIMI', 'grok-cli': 'GROK', 'hermes': 'HERMES' };
      const out = {};
    ...
    // ─── Chat: métodos (Fase 3) ───
    openChat() {
      // Chat livre: concierge conversacional. Gera um chatId novo.
      this.chatId = 'chat-' + Date.now().toString(36);
      this.chatMode = 'run';        // 'run' aqui = concierge (chat-agent)
      this.chatMessages = [];
      this.chatRunEvents = [];
    ...
    // Superfície de orquestração AG-UI: mapeia cada evento de audit em um bloco
    // semântico {icon, title, sub, tone} — quem (empresa/squad/agente), o quê,
    // com custo/passo. Base da timeline viva do modo run.
    runEventView(ev) {
    ...
    // Cabeçalho vivo derivado do stream: empresa/squad/agente atual + custo total.
    get runSummary() {
    ...
    chatRuntime: '',           // '' = auto (host da sessão); ou claude-code/codex/…
    chatFast: false,           // modo rápido/econômico (opt-in); padrão = agêntico
    chatResumeSession: null,   // session_id do runtime para continuar a conversa
    // Envia um turno. Padrão = concierge conversacional (chat-agent). O modo
    // revise/resume só é usado ao "Continuar session" de um run existente.
    async sendChat() {
      const msg = this.chatInput.trim();
      if (!msg || this.chatBusy) return;
      // `!cmd` = comando de shell; `/cmd` = slash command. Interceptar ANTES do
      // concierge (só no modo conversa; em revise/resume a msg vai pro run).
      if (this.chatMode === 'run') {
        if (msg.startsWith('!')) { this.chatInput = ''; return this.runShell(msg.slice(1).trim()); }
        if (msg.startsWith('/')) { this.chatInput = ''; return this.runSlash(msg); }
      }
      ...
      // Pega a referência REATIVA (proxy do array) — mutar o objeto cru não
      // dispararia o re-render do Alpine, e a bolha ficaria em "orquestrando…".
      const asst = this.chatMessages[this.chatMessages.length - 1];
      this.chatBusy = true;
      // Salva no histórico JÁ ao enviar (não só no fim) — Ctrl+R não perde.
      this.saveChatToHistory();
      try { location.hash = `#/chat/${this.chatId}`; } catch {}
      this.$nextTick(() => this.scrollChatBottom());

      // revise/resume = continuar um RUN de produção específico (nrv revise/resume).
      // Caso contrário, o concierge conversacional.
      const isRunMode = this.chatMode === 'revise' || this.chatMode === 'resume';
      if (isRunMode) this.subscribeRun(this.chatId);
      const action = this.chatMode === 'revise' ? 'chat-revise' : (this.chatMode === 'resume' ? 'chat-resume' : 'chat-agent');
      const body = { chat_id: this.chatId, message: msg };
      if (action === 'chat-agent') {
        if (this.chatResumeSession) body.resume_session = this.chatResumeSession;
        if (this.chatRuntime) body.runtime = this.chatRuntime;
        if (this.chatFast) body.fast = true;
      }
      try {
    ...
    // `!<cmd>` — roda um comando de shell arbitrário (livre; localhost + gate).
    async runShell(cmd) {
      if (!cmd) return;
      if (!this.chatId) this.chatId = 'chat-' + Date.now().toString(36);
    ...
    // `/<cmd>` — slash commands (client-side; sem shell).
    runSlash(raw) {
      const [cmd, ...rest] = raw.slice(1).trim().split(/\s+/);
      const arg = rest.join(' ');
      const say = (text) => { this.chatMessages.push({ role: 'system', text }); this.$nextTick(() => this.scrollChatBottom()); };
    ...
    // Despacha um run de produção real (slug + brief) via a action chat-run.
    async runDispatch(slug, brief) {
      if (!this.chatId) this.chatId = 'chat-' + Date.now().toString(36);
      this.chatMessages.push({ role: 'user', text: `/run ${slug} ${brief}` });
      this.chatMessages.push({ role: 'assistant', text: '', events: [], streaming: true });
      const asst = this.chatMessages[this.chatMessages.length - 1];
      this.chatBusy = true;
    ...
    // Segue o job pela SSE de stdout. Formato do servidor: `data: {kind, line?,
    // lines?, code?}` (eventos SEM nome → usar onmessage, não addEventListener).
    followChatJob(jobId, asst, kind) {
      ...
      // Concierge com streaming (NDJSON): tok→texto ao vivo, tool→chip, done→payload.
      const feedConcierge = (line) => {
        let nd; try { nd = JSON.parse(line); } catch { return; }
        if (nd.t === 'tok') { asst.text = (asst.text || '') + nd.v; this.$nextTick(() => this.scrollChatBottom()); }
        else if (nd.t === 'tool') { asst.tools = [...(asst.tools || []), { name: nd.name, cmd: nd.cmd }]; this.$nextTick(() => this.scrollChatBottom()); }
        else if (nd.t === 'done') { doneData = nd; }
        else if (nd.result !== undefined) { doneData = { result: nd.result, session_id: nd.session_id }; }  // formato antigo (não-streaming)
      };
      // errText: mensagem a exibir quando o job termina SEM saída (conexão
      // perdida / timeout). Só aparece se não houver resposta real no buffer.
      const finish = (errText) => {
        clearGrace(); clearTimeout(watchdog);
        es.close();
        asst.streaming = false;
        this.chatBusy = false;
        const joined = buf.join('\n').trim();
        if (kind === 'concierge') {
          // Streaming: asst.text já foi montado pelos tokens; o done traz a
          // resposta canônica (e o session_id p/ continuar). Runtimes sem
          // token-stream (codex/gemini/agy/hermes) mandam tudo de uma vez no done.
          ...
          // Saída crua do shell → bloco de código monospace.
      ...
      // Rede final: se nada sinalizar 'done' (job pendurado), não deixa
      // "orquestrando…" eterno. 6min > timeout do concierge (5min) p/ não matar
      // um turno legítimo lento.
      const watchdog = setTimeout(() => {
        if (this.chatBusy) finish('⚠ sem resposta (tempo esgotado). Recarregue a página e tente de novo.');
      }, 6 * 60 * 1000);
      es.onmessage = (e) => {
        clearGrace(); // chegou dado → conexão viva
        try {
          const d = JSON.parse(e.data);
          // snapshot é o estado autoritativo do stdout-até-agora; numa reconexão
          // do EventSource ele rechega completo → resetar (não acumular duplicado).
          if (d.kind === 'snapshot') {
            buf = Array.isArray(d.lines) ? [...d.lines] : [];
            // Reconexão: reconstrói o texto do concierge a partir do snapshot.
            if (kind === 'concierge') { asst.text = ''; asst.tools = []; doneData = null; for (const l of buf) feedConcierge(l); }
          }
          else if (d.kind === 'line' && typeof d.line === 'string') {
            buf.push(d.line);
            if (kind === 'concierge') feedConcierge(d.line);
            else if (kind === 'shell') { asst.text = '```\n' + buf.slice(-40).join('\n') + '\n```'; this.$nextTick(() => this.scrollChatBottom()); }
            else if (kind === 'run') { asst.text = buf.filter(l => /▶|✓|✗|→/.test(l)).slice(-8).join('\n') || 'orquestrando…'; this.$nextTick(() => this.scrollChatBottom()); }
          }
          else if (d.kind === 'done' || d.kind === 'error') finish();
        } catch {}
      };
      es.onerror = () => {
        if (es.readyState === EventSource.CLOSED) { if (this.chatBusy) finish('⚠ conexão encerrada. Recarregue a página.'); return; }
        // CONNECTING = o EventSource está reconectando sozinho (blip transitório
        // ou servidor morto). Dá 20s de graça: se um snapshot/done rechegar, o
        // onmessage limpa o timer; se NÃO recuperar, o servidor provavelmente
        // reiniciou/morreu → desiste em vez de travar em "orquestrando…" eterno.
    ...
    applyHash() {
      ...
      if (parts[0] === 'chat' && parts[1]) {
        // O hash reflete a conversa ATUAL (sendChat o atualiza). Se já estamos
        // nela, não recarregar — recarregar zeraria as bolhas ao vivo.
