...
export function writePrivateBytes(path: string, bytes: Uint8Array): void { const parent = dirname(path), temporary = join(parent, `.${basename(path)}.${randomUUID()}.tmp`); let descriptor: number | undefined; try { mkdirSync(parent, { recursive: true, mode: 0o700 }); restrictDirectory(parent); descriptor = openSync(temporary, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY, 0o600); restrictFile(temporary); writeFileSync(descriptor, bytes); fsyncSync(descriptor); closeSync(descriptor); descriptor = undefined; renameSync(temporary, path); restrictFile(path); fsyncDirectory(parent); const observed = readFileSync(path); if (!Buffer.from(observed).equals(Buffer.from(bytes))) throw new Error("REREAD_MISMATCH"); } catch (cause) { throw new ServiceIoError("PRIVATE_WRITE", cause); } finally { if (descriptor !== undefined) closeSync(descriptor); rmSync(temporary, { force: true }); } }
