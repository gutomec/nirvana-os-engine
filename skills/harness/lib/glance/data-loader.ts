...
interface Run {
  ...
  resumable: boolean;               // session.json existe com session_id → nrv revise
  session_id: string | null;        // id da conversa do runtime (para --resume)
  session_runtime: string | null;   // runtime que roda a session
}

// Localiza o session.json de um run (espelha revise.ts findSessionFile) e devolve
// os metadados de continuação. Retorna resumable=false quando não há session_id.
function findRunSession(pid: string): { resumable: boolean; session_id: string | null; runtime: string | null } {
  ...
  if (!pid) return none;
  const roots = [
    path.join(process.cwd(), "outputs", pid),
...
export function buildRuns(opts: { since?: string; limit?: number; days?: number } = {}): { runs: Run[]; total: number } {
  ...
  for (const d of dirs) {
    ...
    for (const line of lines) {
      ...
      // Captura o session_id direto do evento agent_executed (mais barato que
      // stat de disco; o findRunSession abaixo cobre o fallback/resumabilidade).
      if (ev.session_id && !r.session_id) { r.session_id = ev.session_id; r.session_runtime = ev.runtime || r.session_runtime; }
      ...
      const evBiz = ev.business_slug ?? (ev as any).business; // alias agêntico (E3)
  ...
  // Resumabilidade: stat de session.json só nos runs retornados (barato). Se o
  // evento já trouxe session_id, é resumível; senão confirma no disco.
  for (const r of sliced) {
    if (r.session_id) { r.resumable = true; if (!r.session_runtime) r.session_runtime = findRunSession(r.trace_id).runtime; continue; }
    const s = findRunSession(r.trace_id);
    r.resumable = s.resumable; r.session_id = s.session_id; r.session_runtime = s.runtime;
  }
