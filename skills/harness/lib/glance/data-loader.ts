...
interface Run {
  ...
  resumable: boolean;               // session.json exists with session_id → nrv revise
  session_id: string | null;        // runtime conversation id (for --resume)
...
export function buildRuns(opts: { since?: string; limit?: number; days?: number } = {}): { runs: Run[]; total: number } {
  ...
  for (const d of dirs) {
    ...
    for (const line of lines) {
      ...
      const evBiz = ev.business_slug ?? (ev as any).business; // agentic alias (E3)
  ...
  // Resumability: stat session.json only for the returned runs (cheap). If the
  // event already carried a session_id, it is resumable; otherwise confirm on disk.
