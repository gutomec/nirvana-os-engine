...
export async function startServer(opts: ServerOptions) {
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      // ─── Static frontend ───
      if (p === "/" || p === "/index.html") {
        const html = readView("index.html").replace("__GLANCE_THEME__", opts.theme);
        return new Response(html, { headers: { "content-type": "text/html; charset=utf-8" } });
      }
      if (p === "/tokens.css") return new Response(readView("tokens.css"), { headers: { "content-type": "text/css" } });
      if (p === "/components.css") return new Response(readView("components.css"), { headers: { "content-type": "text/css" } });
      if (p === "/glance.css") return new Response(readView("glance.css"), { headers: { "content-type": "text/css" } });
      if (p === "/glance.js") return new Response(readView("glance.js"), { headers: { "content-type": "application/javascript" } });
      if (p === "/dag-renderer.js") return new Response(readView("dag-renderer.js"), { headers: { "content-type": "application/javascript" } });
      if (p === "/org-chart-renderer.js") return new Response(readView("org-chart-renderer.js"), { headers: { "content-type": "application/javascript" } });
      if (p === "/graph-renderer.js") return new Response(readView("graph-renderer.js"), { headers: { "content-type": "application/javascript" } });
      if (p === "/chart-renderer.js") return new Response(readView("chart-renderer.js"), { headers: { "content-type": "application/javascript" } });
      if (p === "/awwwards-hero.js") return new Response(readView("awwwards-hero.js"), { headers: { "content-type": "application/javascript" } });
      if (p === "/agent-swimlane-renderer.js") return new Response(readView("agent-swimlane-renderer.js"), { headers: { "content-type": "application/javascript" } });
      if (p === "/agent-workspace-renderer.js") return new Response(readView("agent-workspace-renderer.js"), { headers: { "content-type": "application/javascript" } });
      ...
      if (p === "/api/cost/summary") {
        ...
        return json({ period, scoped_to: projectParam || null, ...aggregator.aggregate(events) });
      }
...
const ACTIONS: Record<string, ActionDef> = {
  ...
  // ── Chat / session actions (Fase 2+3). Lane "chat", sempre --safe + budget. ──
  // O texto do usuário é um ARGV element (Bun.spawn = argv-exec, não shell), então
  // não há injeção de comando; o guarda-corpo restante é --safe (tools limitadas,
  // sem yolo) + teto de budget por turno.
  "chat-run": {
    command: "bun",
    lane: "chat",
    mutating: true,
