...
function aggregate(events, opts = {}) {
  ...
  const bySession = new Map(); // trace_id (=session/task) -> { trace_id, ops, tokens, usd, model, first_ts, last_ts }
  const byProject = new Map(); // project_id (=project folder) -> { project_id, ops, tokens, usd, sessions:Set, last_ts }
  let opsTotal = 0;
  ...
  for (const e of filteredEvents) {
    ...
    // Per session/task: each trace_id is one request/run. Summing the cost of
    // every turn of that session yields what THIS task cost.
    ...
    // Per project (folder): compares spend across projects in the "all" view.
  ...
  const top_expensive_businesses = Array.from(byBiz.values())
    .sort((a, b) => b.usd - a.usd).slice(0, 10);
  // Most recent sessions/tasks first (what was done last goes on top).
