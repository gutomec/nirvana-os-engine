...
export interface StartOpts {
  ...
  scope_mode?: string;
  project_root?: string | null;
  // Concurrency lane. "maintenance" (default) = the single mutating slot
  // (audit-batch/index/…). "chat" = a separate slot, so a chat turn (which
  // also writes files) doesn't collide with maintenance nor vice versa.
