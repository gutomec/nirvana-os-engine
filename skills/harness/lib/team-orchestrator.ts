...
export interface TeamRunArgs {
  ...
  maxBudgetUsd?: number;
  timeoutMs?: number;
  /** Bloco de regras USE_* do usuário (formatRulesForDirective) — anexado ao
   * AUTONOMOUS_DIRECTIVE de cada step para o maestro respeitar ao delegar. */
...
/**
 * Roda pelo cascade RETOMANDO a sessão anterior desta entidade neste projeto.
 *
 * Por que existe: sem isto, a mesma empresa (ou squad) chamada duas vezes no
 * mesmo projeto recomeça fria e reconstrói o que já sabia. O ganho não é
 * velocidade — é o agente continuar sendo o mesmo agente.
 *
 * O fallback é a parte que não pode faltar: o driver passa `--resume <id>` e NÃO
 * trata id inválido. Sessão expirada, apagada pelo CLI ou de outra máquina faria
 * falhar um dispatch que hoje funciona. Então: se o run falhou E tínhamos passado
 * um id, descarta o id e tenta UMA vez fria. Assim o reuso só pode melhorar o
 * resultado, nunca degradá-lo — o pior caso é o comportamento de hoje.
 */
function runWithSession(
  ...
  if (!res.ok && prior) {
    // A sessão pode ter morrido fora do nosso controle. Não dá para distinguir
    // "resume inválido" de "a tarefa falhou" pelo texto do erro de forma
    // confiável em 6 runtimes, então a política é simples e segura: uma
    // tentativa fria antes de desistir.
    appendAudit({
...
function squadCloneInjection(brief: string): { block: string; decision: string; missingClones: string[] } {
  ...
  // Clone pedido e inexistente não derruba o squad — mas degradar em SILÊNCIO
  // seria pior que falhar: o squad produziria sem o DNA e ninguém saberia. Mesma
  // política do dispatch.ts: a degradação é RUIDOSA — bloco explícito no próprio
  // prompt, campo no retorno (o chamador emite o evento de auditoria) e ordem ao
  // agente de reportar a ausência no entregável.
...
function runMandatorySquad(squadSlug: string, args: TeamRunArgs): StepResult {
  ...
  for (const slug of cloneInj.missingClones) {
    appendAudit({
      ...
      business_slug: args.slug, squad_slug: squadSlug, reason: "mind_clone_not_found", slug_requested: slug,
    }, args.projectRoot);
  }
...
Arquivos no diretório acima. Não printe sumário — entregue arquivos. Termine quando o trabalho estiver pronto para o synthesizer integrar.`;
  ...
  const res = runWithSession("squad", squadSlug, args, {
