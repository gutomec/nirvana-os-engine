/**
 * anomaly-detector.ts — z-score over per-business / per-squad KPIs and
 * detection of individual traces outside the statistical norm.
 *
 * Design principles:
 *  - Does not fire for new entities (sample size < min_samples)
 *  - Per-entity cooldown avoids noise (same anomaly does not fire 2× in N min)
 *  - Deterministic: given the same input, always the same output
 *  - No external sources — operates on the trace-builder result
 */
...
export interface AnomalyThresholds {
  ...
  cooldown_minutes: number;  // default 60 (only relevant when used with persisted state)
}
