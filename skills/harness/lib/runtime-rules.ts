...
export interface RuntimeDecision {
  runtime: Runtime;
  /** flag = CLI (--exec=<rt>/--runtime); brief = menção explícita no texto do
   *  brief ("use o agy para..."); rule = USE_*; default = host da sessão. */
  source: "flag" | "brief" | "rule" | "default";
  rule?: RuntimeRule;
  method?: "bm25" | "agentic";
  score?: number;
  /** Trecho do brief que citou o runtime (source === "brief"). */
  mention?: string;
...
const EXEC_RUNTIMES: ReadonlyArray<Runtime> = ["claude-code", "codex", "gemini-cli", "antigravity-cli", "kimi-cli", "grok-cli"];
...
// Menção EXPLÍCITA de runtime no brief ("Use o agy para pesquisar...").
// Exige um cue instrumental antes do nome (use/via/pelo/com o/no/using/with...)
// para não confundir CONTEÚDO com INSTRUÇÃO — um brief SOBRE a estátua de
// Hermes não pode rotear pro Hermes. Nome do runtime sozinho não basta.
...
const MENTION_CUE =
  /\b(?:use|usa|usando|utilize|utilizando|rode|rodando|execute|executando|despache|via|pelo|pela|com|no|na|using|with|through|run(?:ning)? (?:it )?on|on)\s+(?:o\s+|a\s+|the\s+)?((?:agy|antigravity|codex|gemini|claude|hermes)(?:[- ](?:cli|code))?)\b/gi;

/** Detecta menção instrumental a um runtime no brief. null quando: nenhuma,
 *  ou mais de um runtime distinto citado (ambíguo — não adivinha). */
...
export function decideRuntime(opts: {
  ...
  // Menção explícita no BRIEF ("Use o agy para pesquisar...") = o usuário
  // falando diretamente. Vence vetos e regras (config); perde só para a flag.
  // hermes citado → só delegação (o maestro vê via diretiva); segue o fluxo.
