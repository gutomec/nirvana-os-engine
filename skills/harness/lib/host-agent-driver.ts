...
function runAntigravity(opts: RunHeadlessOpts): RunHeadlessResult {
  ...
  if (opts.sessionId) args.push("--resume", opts.sessionId);
  if (opts.model) args.push("--model", opts.model);
  // TODO(antigravity): autonomous-mode flag not confirmed in research
  // ((base de conhecimento interna) §5.3 — approval modes: Request Review /
  // Proceed-in-Sandbox / Always-Proceed). Confirm with `agy --help` once
  // authenticated, then map: opts.yolo !== false → Always-Proceed.
  ...
  // Keep full stdout — Antigravity uses the Google backend, so the cost-
  // estimator can extract tokens from the embedded usage block (same shape
  // as gemini-cli or a `usage` field on the parsed object).
  const result = rawStdout;
