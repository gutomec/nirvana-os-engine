/**
 * trace-builder.ts — groups `audit.jsonl` events into span trees by
 * `trace_id`. Streaming-friendly: reads files line-by-line without
 * materializing the whole JSON in memory.
 *
 * Produces a Jaeger/Honeycomb-like structure consumable by a UI:
 *
 *   {
 ...
 *   }
 *
 * Phase 2 of nirvana-evolution. See docs/nirvana-evolution/README.md.
 *
 * Findings this module addresses (from the Phase 0 baseline):
 *
 *   - Trace_id fragmentation: the Claude Code hook emits `cost_emission` with
 *     `session_id` as trace_id, while the harness emits `dispatch_*` with its
 *     own trace_id. `correlateTraces()` covers the first level of that problem
 *     by associating session_id ↔ harness trace_id when both appear in the same
 *     tight time window (a declared heuristic — no magic inference attempted).
 */
...
function summarize(traceId: string, events: Record<string, unknown>[]): TraceTree {
  ...
  for (const sp of spans) {
    const p = sp.payload;
    // Accepts the bare alias (`business`/`squad`) the agentic maestro writes
    // via echo (SKILL.md), besides the canonical `business_slug`/`squad_name`
    // of the CLI path. Since readers re-read the raw JSONL on every run, this
    // retroactively recovers all history already on disk (E3).
    // An empty string is not an entity. An event written without context
    // carried `business: ""` and inflated the businesses/squads count by +1,
    // which showed up as a phantom participant in the trace summary.
    const biz = p.business_slug ?? p.business;
    if (typeof biz === "string" && biz !== "") businessSet.add(biz);
    const sqd = p.squad_name ?? p.squad;
    if (typeof sqd === "string" && sqd !== "") squadSet.add(sqd);
