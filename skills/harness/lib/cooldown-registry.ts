 ...
 * them. Fail-open on lock trouble: losing one cooldown update beats failing
 * the dispatch. */
function mutate(projectRoot: string, fn: () => void): void {
  try {
    withLock(fileFor(projectRoot), fn);
  } catch (e) {
    console.error(`[cooldown-registry] lock unavailable (${(e as Error)?.message ?? e}) — applying update unlocked.`);
    fn();
  }
}
