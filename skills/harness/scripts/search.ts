...
// MIND-CLONES — scan ~/businesses/_library/dna/<slug>/MANIFEST.yaml (flat layout)
if (!kindArg || kindArg === "mind-clone") {
  const dnaRoot = path.join(os.homedir(), "businesses/_library/dna");
  if (fs.existsSync(dnaRoot)) {
    for (const slug of fs.readdirSync(dnaRoot)) {
      const clonePath = path.join(dnaRoot, slug);
      ...
      const category = m.match(/category:\s*["']?([^"'\n]+)/)?.[1]?.trim() || "";

      const fields = [
        { text: slug, w: 2.0, name: "slug" },
        { text: displayName, w: 1.8, name: "name" },
        { text: category, w: 1.5, name: "category" },
