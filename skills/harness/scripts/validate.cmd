@echo off
REM validate.cmd — Windows wrapper. Runs the .ts via Bun (or Node 22+ as fallback).
...
if %ERRORLEVEL% EQU 0 (
  node --experimental-strip-types --no-warnings "%~dp0validate.ts" %*
  exit /b %ERRORLEVEL%
)

echo [validate] FAIL: neither 'bun' nor 'node' found on PATH. >&2
