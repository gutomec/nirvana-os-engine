...
export interface DigestInput {
  ...
  /**
   * Which registry FILES parsed. An empty library and an unreadable registry
   * both yield zero entries, and treating them alike made the first command a
   * new install is told to run exit 1 with "run `nrv index` first" — advice for
   * the command the user had just run. A content-free engine is the DESIGNED
   * state, so emptiness is success; only an unreadable registry is a failure.
   * Optional so existing fixtures stay valid.
   */
  readable?: { businesses: boolean; squads: boolean; mindClones: boolean };
}
...
function renderAt(input: DigestInput, level: 0 | 1 | 2 | 3 | 4, generatedAt: string): string {
  ...
  const lines: string[] = [
    ...
    `  businesses registry: ${input.registryPaths.businesses} (each entry's manifest_path → full business.yaml, and beside it employees/*.md)`,
    `  squads registry: ${input.registryPaths.squads} (each entry's manifest_path → full squad.yaml, and beside it agents/*.md, tasks/*.md, workflows/*)`,
    `  mind-clones registry: ${input.registryPaths.mindClones} (each entry's dir/persona_files → MANIFEST.yaml, AGENT.md)`,
    "  a manifest is a claim; the files beside it are the evidence. When finalists look alike here, open them and compare what actually executes.",
...
export function loadDigestInput(p: { businessesRegistry: string; squadsRegistry: string; mindClonesRegistry: string }): DigestInput {
  ...
  return {
    ...
    readable: { businesses: biz !== null, squads: sq !== null, mindClones: cl !== null },
  };
}
...
if (import.meta.main) {
  ...
  const isEmpty = !Object.keys(input.businesses).length
               && !Object.keys(input.squads).length
               && !Object.keys(input.mindClones).length;

  // Only a registry that would not PARSE is a failure. A registry that parses
  // and holds nothing is a fresh, content-free install — the engine's designed
  // starting state — and it gets a valid empty digest instead of an error
  // telling the user to run the command they just ran.
  if (!readable.businesses && !readable.squads && !readable.mindClones) {
  ...
  if (isEmpty && !quiet) {
    console.log("[build-routing-digest] library is empty — writing an empty digest (create a business, squad or mind-clone, then re-run `nrv index`).");
  }
