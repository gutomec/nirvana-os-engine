...
if (!pack) {
  console.error(`no mind-clones declared for grading`);
  process.exit(2);
}

// A clone declared but absent from the library is not a fidelity failure of the
// artifact — the agent never received that DNA. So the absence is announced,
// but never becomes a FAIL. Previously it vanished inside a pack without the
// clones (or a `null`) and the gate stayed mute about what was missing.
if (pack.missing_clones.length) {
  console.error(
    `⚠ mind-clone(s) not in the installed library, NOT graded: ${pack.missing_clones.join(", ")}\n` +
    `  Create them with the \`fabrica-de-genios\` squad, capability \`knowledge_management.mind_clone_generation_pipeline.execute\`.`
  );
}
if (pack.mind_clones.length === 0) {
  console.error(
    `nothing to grade: every declared mind-clone is absent from the library. ` +
    `This is a library gap, not a fidelity failure — no verdict emitted.`
  );
  process.exit(2);
}
...
if (pack.deterministic_signals.overall_marker_density < SHORTCIRCUIT_FLOOR) {
  const out = {
    ...
    // Shipped in the JSON so whoever reads the verdict knows that part of the
    // declared clones was never even evaluated — and why.
    missing_clones: pack.missing_clones,
  };
