...
if (wantExec) {
  ...
  // Step 6.6 — optional HTML report: render every markdown produced in the
  // project into one self-contained HTML file (marked, no external tools). Lands
  // in deliverables/ so the --zip bundle carries it.
  if (wantHtml) {
    ...
    const h = spawnSync("bun", [htmlBuild, "--project", projDir, "--output", htmlOut, "--title", `Relatório — ${slug}`], { encoding: "utf8", stdio: "inherit" });
