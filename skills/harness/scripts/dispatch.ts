...
// A flag --runtime foi DADA pelo usuário? (arg() não distingue flag de default;
// flag explícita SEMPRE vence as regras USE_* — regra só vence o default.)
...
// Anti-stub: arquivos com menos de 200 bytes são tratados como rascunho — EXCETO
// quando o próprio brief nomeia o arquivo (um haicai.md legítimo tem ~60 bytes;
// o pedido explícito do usuário é evidência mais forte que o tamanho). Vazio
// (0 bytes) nunca passa.
const MIN_DELIVERABLE_BYTES = 200;
function briefNamedFiles(briefText: string): Set<string> {
  const out = new Set<string>();
  // nome.ext com extensão alfabética curta ("haicai.md", "relatorio.html");
  // "v7.0.0" não casa (extensão exige letras).
  for (const m of briefText.matchAll(/[\p{L}\p{N}_-]+\.[a-z]{1,5}\b/giu)) out.add(m[0].toLowerCase());
  return out;
}
function isDeliverable(f: string, named: Set<string>): boolean {
  try {
    const size = fs.statSync(f).size;
    return size >= MIN_DELIVERABLE_BYTES || (size > 0 && named.has(path.basename(f).toLowerCase()));
  } catch { return false; }
}
function nonStubText(dir: string, named: Set<string>): string[] {
...
// ── Regras USE_* do usuário (roteamento por runtime em linguagem natural) ──
// Precedência: flag explícita (--exec=<rt> | --claude-code | --runtime dado)
// > regra USE_* > default = o runtime que o USUÁRIO JÁ ESTÁ USANDO (host da
// sessão) > claude-code. O LLM_CASCADE continua dono da resiliência (cota).
...
if (runtimeDecision.source === "brief") {
  ...
  appendAudit({
    event: "routing_rule_applied", project_id: projectId || null,
  ...
  // Vetos NOT_USE_* mudaram/limitaram a escolha sem regra positiva vencedora.
  ...
  appendAudit({
    event: "routing_rule_vetoed", project_id: projectId || null,
...
// DELEGAR sub-tarefas ("" quando não há regras = no-op).
...
if (autoMode && routingMode === "fast") {
  ...
  // O roteador agêntico LEU as regras USE_* do usuário; se sugeriu um runtime
  // e não há flag explícita NEM menção direta no brief (que é mais forte que a
  // sugestão do LLM), a sugestão semântica sobrepõe o match BM25.
...
if (wantExec) {
  // Runtime final: a decisão flag > regra USE_* > host atual (já computada).
  ...
  const namedInBrief = briefNamedFiles(brief);
  const produced = listFiles(oroot).filter(f => isDeliverable(f, namedInBrief));
  ...
  let textFiles = nonStubText(oroot, namedInBrief);
  ...
  while (!gate.pass && revUsed < maxRevisions) {
    ...
    const rr = runHeadless({
      ...
      sessionId: res.sessionId || undefined, appendSystemPrompt: AUTONOMOUS_DIRECTIVE + rulesDirective,
      maxBudgetUsd: maxBudget ? parseFloat(maxBudget) : undefined,
    ...
    textFiles = nonStubText(oroot, namedInBrief);
