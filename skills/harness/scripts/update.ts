...
// `nrv update`. Best-effort: network down or no PROVENANCE never breaks the update.
...
async function updateFromRelease(): Promise<never> {
  ...
  // tar with cwd at the tarball's dir and RELATIVE "/" paths: an absolute
  // Windows path (C:\...) has ":" and Git Bash's GNU tar treats it as a remote
  // host ("tenta conectar no C:"). Relative has no ":" and works on GNU and bsdtar.
