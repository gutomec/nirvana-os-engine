...
// SECTION 3: REGISTRIES — mesma resolução de path do indexer (scope-aware via
// bun-helpers). O path fixo $HOME/.squads-registry.json era o local LEGADO de
// antes da migração para ~/.nirvana: o doctor dizia "missing" logo depois de um
// `nrv index` bem-sucedido, porque cada um olhava para um lugar.
...
// Um projeto dispatchado deixa um dir com QUALQUER destes marcadores: businesses/
// (dispatch de empresa), squads/ (dispatch de squad), HANDOFF.json, deliverables/
// ou brief.md. Contamos por nome de projeto (dedup entre roots). OS-safe: segmentos
// separados no path.join (Windows).
const OUTPUT_ROOTS = [
...
// SECTION 5: AUDIT
// Reporta a atividade REAL — não engana. Antes só olhava HOJE no escopo do cwd e,
// se vazio, chutava "no dispatches yet?" (falso quando o dispatch foi noutro dia,
// noutra pasta, ou quando o agente — ex.: Codex — não rodou `nrv audit emit`).
const today = new Date().toISOString().slice(0, 10);
const { harnessLogsDir } = require(path.join(SKILLS, "_shared/lib/log-paths.ts"));
// Escopos possíveis: projeto (cwd) + global (~/.harness-logs).
...
if (todayLines.length) {
  ...
  for (const l of todayLines) { try { const e = JSON.parse(l); counts[e.event] = (counts[e.event] || 0) + 1; } catch {} }
...
// Conta projetos dispatchados por marcador REAL (empresa OU squad OU HANDOFF/brief),
// não só `businesses/` — antes o dispatch de squad (starter pack é squad-heavy)
// zerava e o rótulo mentia "HANDOFF.json".
