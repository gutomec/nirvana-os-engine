...
/** Lines the viewer could not parse. Reported, never silently dropped: a
 *  summary that hides them tells the reader the log is complete when it is not. */
...
if (all) {
  ...
  if (unreadableLines) console.log(c("yellow", `  ⚠ ${unreadableLines} unreadable line(s) skipped — this summary does not account for them`));
  console.log("");
...
if (events.length === 0) {
  ...
  if (unreadableLines) console.log(c("yellow", `  ⚠ but ${unreadableLines} line(s) in the log could not be parsed — "none found" may be wrong.`));
  process.exit(0);
}
