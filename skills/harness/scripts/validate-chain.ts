...
function loadEvents(projectId: string): { events: AuditEvent[]; sources: string[]; unreadable: { file: string; count: number }[] } {
  const events: AuditEvent[] = [];
  const sources: string[] = [];
  // An integrity checker must not confuse "event absent" with "event
  // unreadable". A dropped malformed line used to become a reported GAP (or,
  // worse, silence), which sends the reader hunting for a dispatch that did
  // happen. Count them per file and surface them.
  const unreadable: { file: string; count: number }[] = [];
  const seen = new Set<string>();
  const addEvent = (e: AuditEvent) => {
    // Replay-aware dedup (routing-360 Phase 4): dispatch.ts replays pre-projDir
    // events from the launch-cwd root into the project root, flagged
    // `replayed_from_global: true` and carrying the ORIGINAL ts — so the copy
    // and the source produce the same key here and count as ONE event.
      ...
      for (const biz of fs.readdirSync(bizDir)) {
        ...
        if (fs.existsSync(auditPath)) {
          ...
          for (const l of lines) {
            try { addEvent(parseAuditLine(l)); } catch { bad++; }
          }
          if (bad) unreadable.push({ file: auditPath, count: bad });
  ...
  return { events, sources, unreadable };
}
...
function validateProject(projectId: string, opts: { strict: boolean; verifyDisk?: boolean; minBytes?: number }): ValidationResult {
  const { events, sources, unreadable } = loadEvents(projectId);
  if (events.length === 0) {
    return {
  ...
  // Tolerate the open x_ namespace (lib/audit.js): an event recorded under a
  // mismatched enum (older/newer audit.js) lands as `x_<name>` — for required-
  // event checks, both spellings count as the event having happened.
  const count = (name: string): number => (counts[name] || 0) + (counts[`x_${name}`] || 0);

  const gaps: string[] = [];

  // Lines we could not parse are reported BEFORE any missing-event verdict:
  // "not found" and "not readable" are different failures, and only one of
  // them means the work did not happen.
  for (const u of unreadable) {
    gaps.push(`${u.count} unreadable line(s) in ${u.file} — the verdict below cannot account for them`);
  }

  // Required events
  if (!count("brief_received")) gaps.push("missing event: brief_received (brief-business.ts not run?)");
  if (!count("dispatch_business")) gaps.push("missing event: dispatch_business (maestro did not emit?)");
  if (count("handoff_phase_advanced") < 2) {
    gaps.push(`handoff_phase_advanced count = ${count("handoff_phase_advanced")}; expected ≥ 2 (plan→execute + execute→complete)`);
  }
  if (!count("verify_passed")) {
    gaps.push("missing event: verify_passed (verify-deliverable.ts not run, or returned FAIL/INDETERMINATE)");
  }
  if (opts.strict && !count("gate_passed")) {
