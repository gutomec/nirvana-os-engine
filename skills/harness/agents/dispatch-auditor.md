...
2. **`unresolvable_target`** — The plan references a **squad or business**
   slug that is not in the matching registry.
   Fix: "Pick a target that exists, or emit no_match and ask the user."

   NOT for mind-clones. A missing mind-clone never blocks a dispatch — see
   the warning list below. Blocking on it would resurrect, one floor up, the
   hard failure that `injectMindClones()` deliberately stopped doing.
...
3b. **`mind_clone_not_in_library`** — The plan names a mind-clone that is not
   installed. This is a **warning, never critical**: the dispatch proceeds,
   `injectMindClones()` emits `mind_clone_missing_degraded` and injects a block
   telling the agent it does NOT carry that DNA, and the agent is instructed
   not to claim clone fidelity it never loaded.
   Fix: "Proceed, and report to the user that the specialist is not in the
   library. If the method matters, offer to create the clone via squad
   `fabrica-de-genios`, capability
   `knowledge_management.mind_clone_generation_pipeline.execute`."
