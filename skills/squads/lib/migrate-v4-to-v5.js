...
function migrateSquad(slug, opts = {}) {
  ...
  // Portable form: never bake the migrating user's absolute home into the
  // manifest — squads ship inside packs, so an absolute path would leak the
  // author's local directory layout to every buyer. `~` resolves to whichever
  // user installs the squad.
  const homePrefix = os.homedir();
  const portableSrc = srcDir.startsWith(homePrefix)
    ? '~' + srcDir.slice(homePrefix.length)
    : srcDir;
  sanitized.legacy = {
    ...(sanitized.legacy || {}),
    v4_path: portableSrc,
