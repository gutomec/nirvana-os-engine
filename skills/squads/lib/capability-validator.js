...
// Payload limits — single source of truth is _shared/validators/limits.ts (the same
// LIMITS the zod validators use). Load it so this fast pre-check never drifts from
// the authoritative validator again. Falls back to the canonical default (1500, NOT
// the old 500) if the .ts module can't be required in this runtime.
let LIMITS = {};
try {
  LIMITS = require(path.join(SKILLS_ROOT, '_shared', 'validators', 'limits.ts')).LIMITS || {};
} catch (_e) { /* keep {} → fall back to canonical defaults below */ }
const CAP_DESCRIPTION_MAX = LIMITS.capability_description_max ?? 1500;
...
function structuralCapabilityCheck(cap, catalogDomains, options) {
  ...
  if (typeof cap.description !== 'string' ||
      ...
      cap.description.length > CAP_DESCRIPTION_MAX) {
    errors.push(`description must be a string between 20 and ${CAP_DESCRIPTION_MAX} chars`);
  }
