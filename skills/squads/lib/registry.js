...
// Registry payload discipline for the squad-level description (routing-360
// Phase 2.1). squad.yaml's description has no dedicated cap in limits.ts
// (SquadManifestSchema: min 20, no max); 1500 mirrors the default of
// LIMITS.capability_description_max. Hardcoded because this file is
...
function scan(roots) {
  ...
  for (const root of inputRoots) {
    ...
    for (const manifestPath of manifests) {
      ...
      // A name collision in the SAME root is ambiguous (OS order decides the
      // winner): warn (E5e). A cross-root collision is a deliberate override
      // (LEGACY → SQUADS_DIR → cwd), documented — no warning to avoid noise.
...
function build(roots) {
  ...
  for (const entry of entries) {
    ...
    // Aggregates the capabilities' discovery fields at the SQUAD level. Without
    // this, squad-level consumers (the concierge catalog, per-squad matching)
    // only see domains + ids and do NOT see what the squad actually DOES
    // (e.g.: brandcraft creates PDF/carousel/video/site — but that lived only
    // inside the capabilities). Businesses already aggregate; squads did not.
    ...
    squads[m.name] = {
      ...
      // Routing signal (routing-360 Phase 2.1): squad-level description so
      // squad-level consumers (router corpus, `nrv search`, concierge catalog)
      // see what the squad IS, not only its domains + capability ids.
      description: typeof m.description === 'string' ? m.description.slice(0, SQUAD_DESCRIPTION_MAX) : '',
