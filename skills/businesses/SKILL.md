...
|---|---|---|
| **CREATE**: create, new business, scaffold, init | `*business init <name>` | §Wizard flow (neste arquivo — `references/01-creation.md` não existe) |
...
> **Este fluxo É o criador de businesses do sistema** — criação é trabalho do
> engine, executado agenticamente por esta skill, nunca despachado a um squad
> criador. A meta é a melhor empresa possível para a função, não uma válida.

**Round 0 — Arqueologia + pesquisa (antes de perguntar qualquer coisa)**
- Arqueologia de intenção: que dor recorrente a empresa serve, quem consome os
  outputs, o que já existe no portfólio que cobre parte disso
  (`nrv find --no-amplify` com 3-5 briefs hipotéticos — empresa nova que rouba
  território de empresa existente nasce errada; o resultado dita `auto_routes`
  e fronteiras).
- Pesquisa de domínio (web, obrigatória): práticas, ferramentas e serviços
  ATUAIS do setor, com data e fonte — viram o conhecimento dos employees e o
  vocabulário real de `domains`/`example_briefs`/keywords (PT e EN).
- Mind-clones por NECESSIDADE: para cada papel previsto, busque a biblioteca
  (`nrv find-clone "<necessidade do papel>"`) e pré-selecione
  `assigned_mind_clones` com lastro — employee sem clone certo é persona
  genérica. Biblioteca vazia (install limpo do engine, sem pack)? Dois caminhos
  honestos: criar o clone antes (`_shared/MIND_CLONE_CREATION_PIPELINE.md`) ou
  seguir sem clone declarando o employee como persona construída, nunca como
  clone que não existe.
...
**Round 5 — Gate de prontidão (obrigatório, depois do registry)**
- Passe de otimização: releia cada employee como revisor hostil — persona
  genérica, papel sem entregável claro ou conhecimento que o Round 0 pesquisou
  e o employee não usa são defeitos; corrija antes de declarar pronto.
- Gate de roteamento: cada `example_brief` do `business.yaml` deve rotear de
  volta para esta empresa em 1º (`nrv find --no-amplify "<example_brief>"`),
  e 2-3 briefs de SINTOMA (como o dono escreveria em pânico, PT) também.
  Confirme que os briefs-casa das empresas vizinhas do Round 0 continuam com
  as donas. Se não rotear, o defeito é do bloco de descoberta — nunca "do
  roteador".
