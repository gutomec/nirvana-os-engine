...
import { run, paths, parseArgs, EXIT, BUN_BIN } from "../../_shared/lib/bun-helpers.ts";
...
// Argv array — no shell, so Windows paths (C:\...\ with trailing separators)
// never corrupt via cmd.exe quoting.
const cmdArgs = [REGISTRY_LIB, "rebuild"];
if (roots.length > 0) cmdArgs.push("--roots", ...roots);
cmdArgs.push("--output", targetRegistry);
if (quiet) cmdArgs.push("--quiet");
const r = run(BUN_BIN, cmdArgs, {
  silent: false,
  env: {
