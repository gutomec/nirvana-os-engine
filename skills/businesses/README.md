...
```bash
# Clone or copy ~/.nirvana/skills/businesses/ to your machine
mkdir -p ~/businesses
bun ~/.nirvana/skills/businesses/scripts/list-businesses.ts   # auto-bootstraps registry
```
...
```bash
bun ~/.nirvana/skills/businesses/scripts/init-business.ts my-startup --template solo
```
...
```bash
bun ~/.nirvana/skills/businesses/scripts/validate-business.ts ~/businesses/my-startup
```
...
Schemas live centralized in `~/.nirvana/skills/_shared/schemas/`:
...
Validators in `~/.nirvana/skills/_shared/validators/validators.{ts,py}` (Zod + Pydantic v2 mirrors).
...
path.insert(0, '~/.nirvana/skills/businesses/lib')
