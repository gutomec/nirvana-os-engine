...
/** Clone pedido e inexistente NÃO derruba o employee — mas degradar em SILÊNCIO
 *  seria pior que falhar: o funcionário produziria sem o DNA e ninguém saberia.
 *  Mesma política do dispatch.ts e do team-orchestrator.ts: a degradação é
 *  RUIDOSA — este evento de auditoria, mais o bloco explícito no próprio prompt. */
...
type CloneInjection = {
  ...
  decision: string;
  /** Clones pedidos que não existem como clone instalado. Vazio no caso normal.
   *  Antes disto eles eram descartados em silêncio: o funcionário rodava sem o
   *  DNA e nem ele nem o dono ficavam sabendo. */
...
function resolveClonesByPriority(args: BuildArgs, employeeContent: string, bizDir: string): CloneInjection {
  ...
  const push = (slug: string, reason: string): boolean => {
    ...
    // Não resolveu = clone pedido que não existe na biblioteca. Registra em vez
    // de descartar — quem consome transforma isso em aviso ruidoso. Teto de
    // MAX_INJECT e duplicata já saíram acima, então aqui a causa é só ausência.
    if (!p) {
...
export function buildEmployeePrompt(args: BuildArgs): string {
  ...
  if (args.include_dna !== false) {
    ...
    // Degradação ruidosa: o funcionário roda, mas sabendo (e dizendo) que NÃO
    // carrega o DNA daquela pessoa. Mesmo texto do dispatch.ts / team-orchestrator.ts.
    if (inj.missingClones.length) {
      const list = inj.missingClones.join(", ");
      dnaContent += `\n\n--- MIND-CLONE AUSENTE: ${list} ---\n\n` +
