 ...
 *
 * Under Bun this file DELEGATES to the canonical TypeScript driver (see the
 * block right below), so every consumer that requires the .js first
 * (quality-judge.js, host-agent-retry.js, resume-project.ts) still gets the
 * unified implementation — adapters, watchdog fixes, prompt delivery.
 *
 * The inline implementation that follows is ONLY the legacy fallback for
 * plain `node` callers (Node does not strip TypeScript natively). It covers
 * the light layer (detectHost/callHostAgent/callHostAgentAsync) with the
 * pre-unification adapter set and no stall watchdog. The Bun-only project
 * contract makes this path exceptional; do not extend it — extend the .ts.
 */

'use strict';

// Bun can require() the canonical TS driver directly. CJS modules run inside
// a function wrapper, so a top-level return cleanly short-circuits the file.
if (typeof Bun !== 'undefined') {
  try {
    module.exports = require('./host-agent-driver.ts');
    return;
  } catch (_e) { /* fall through to the inline legacy implementation */ }
}
...
// System model (CJS mirror of _shared/lib/system-model.ts). Without
// propagating this, audit-consensus/verifier spawned `claude -p` on the CLI
// default (sonnet) instead of inheriting the session's fable/opus. Keep in sync.
