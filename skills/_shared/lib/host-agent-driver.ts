...
const RUNTIMES: RuntimeAdapter[] = [
  ...
  {
    // Pi Coding Agent (`pi`, pi.dev) — harness minimalista multi-provider
    // (15+ providers, incluindo modelos locais via Ollama/models.json).
    // Print mode (-p) devolve texto puro; --no-approve ignora extensões locais
    // do projeto (sem TTY não há como responder o trust prompt).
    name: "pi",
    cli: "pi",
    buildArgs(persona, userMsg) {
      // Persona via --append-system-prompt (nativo, confirmado no pi 0.82.1).
