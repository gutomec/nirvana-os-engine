...
function loadPaths(): Record<string, string> {
  ...
  _paths = {
    ...
    // "dna" e "mind-clones" são O MESMO conceito, com nomes diferentes por
    // camada — não são dois acervos:
    //   pack (transporte)   mind-clones/<slug>/
    //   escopo de projeto   <projeto>/.nirvana/mind-clones/<slug>/
    //   escopo global       ~/businesses/_library/dna/<slug>/   ← este
    // O install mapeia o primeiro no último (install-content.ts: mind-clones →
    // DNA_DIR). Layout sempre FLAT (um dir por slug); a categoria é metadado em
    // .pack-categories.json, nunca path. Procurar clones por "mind-clones" no
    // disco de uma instalação global não acha nada — eles estão aqui.
