...
function loadPaths(): Record<string, string> {
  ...
  _paths = {
    ...
    // "dna" and "mind-clones" are THE SAME concept, with different names per
    // layer — not two collections:
    //   pack (transport)    mind-clones/<slug>/
    //   project scope       <project>/.nirvana/mind-clones/<slug>/
    //   global scope        ~/businesses/_library/dna/<slug>/   ← this one
    // Install maps the first onto the last (install-content.ts: mind-clones →
    // DNA_DIR). Layout is always FLAT (one dir per slug); the category is
    // metadata in .pack-categories.json, never a path. Searching the disk of a
    // global install for "mind-clones" finds nothing — they live here.
    ...
    ROUTING_DIGEST_PATH: process.env.ROUTING_DIGEST_PATH || path.join(NIRVANA_HOME, ".nirvana", ".routing-digest.md"),
...
/**
 * Argv-based exec — NO shell, so paths never need quoting. Prefer this over
 * `exec()` when any argument is a filesystem path: on Windows `exec()` goes
 * through cmd.exe and shell-quoting a `C:\...` path (e.g. via JSON.stringify)
 * doubles backslashes and a trailing separator escapes the closing quote,
 * corrupting argv. `run(bin, [args])` passes argv verbatim on every OS.
 */
export function run(bin: string, args: string[], opts: { cwd?: string; env?: Record<string, string>; timeoutMs?: number; silent?: boolean } = {}): ExecResult {
  ...
  if (r.error) return { ok: false, stdout: r.stdout || "", stderr: r.stderr || "", code: null, error: r.error.message };
  return { ok: r.status === 0, stdout: r.stdout || "", stderr: r.stderr || "", code: r.status ?? null };
}
...
// EEXIST-tolerant: on Windows Bun can throw EEXIST even with recursive:true
// (and existsSync does not see junctions properly). On Mac/Linux the catch
// never fires.
