...
// Extração de tarball com cwd + paths RELATIVOS com "/": um path absoluto do
...
/**
 * Diretório da biblioteca de mind-clones no escopo pedido. Os clones ficam
 * FLAT dentro dele (um dir por slug) — a categoria é metadado em
 * `.pack-categories.json`, nunca path. É o contrato que `index-clones.ts`
 * (que varre um nível só) e `install-content.ts` já seguem.
 */
...
/**
 * Funde `{slug: categoria}` no `.pack-categories.json` da biblioteca. É o mapa
 * que o `index-clones.ts` lê para preencher `pack_category` — sem alguém
 * escrevê-lo, o campo nunca saía de null em nenhum fluxo de install.
 */
function mergePackCategories(libraryDir: string, entries: Record<string, string>): void {
  ...
  if (existsSync(file)) {
    ...
    } catch { /* corrompido/ilegível → reescreve do zero */ }
  }
...
/** Diretórios-container que NÃO são categoria (só embalagem do pack/biblioteca). */
const NON_CATEGORY_DIRS = new Set(["mind-clones", "mind_clones", "clones", "dna", "_library", "_root", ".", "/", ""]);

/**
 * Categoria de drive do clone — METADADO para `.pack-categories.json`, nunca
 * path (o clone é instalado flat). `null` = sem categoria conhecida.
 */
function inferMindCloneCategory(dir: string): string | null {
  ...
  // Packs reais são flat (`<pack>/mind-clones/<slug>/`), então o pai costuma ser
  // só a embalagem — devolver isso gravaria "mind-clones" como categoria.
...
export async function install(opts: InstallOptions): Promise<InstallResult> {
  ...
  try {
    ...
    // Categoria é METADADO (vai para .pack-categories.json), não path: o clone
    // é instalado flat, que é o único layout que o index-clones enxerga.
...
async function installPack(packDir: string, opts: InstallOptions, manifest: InstallManifest, installId: string, originUri: string): Promise<InstallResult> {
  ...
  const claimedTargets = new Map<string, string>(); // targetDir → item.path que o reivindicou
  for (const k of ["businesses", "squads", "mind-clones"] as const) {
    ...
    for (const item of list) {
      ...
      try {
        ...
        // Layout flat: a categoria vira metadado, não subdiretório.
        ...
        // Flat expõe colisão de slug dentro do próprio pack (antes ficava
        // mascarada por categorias diferentes). Falha explícita > sobrescrita.
  ...
  // Categorias dos clones do pack → metadado da biblioteca (antes do reindex,
  // que é quem lê o mapa para preencher `pack_category`).
