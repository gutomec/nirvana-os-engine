...
async function loadExtractor(): Promise<((t: string, o: unknown) => Promise<{ data: Float32Array }>) | null> {
  ...
  _extractorPromise = (async () => {
    try {
      ...
      // Hardening: nunca carregar modelo do filesystem local (./models do cwd) —
      // fecha o vetor de path-traversal/model-poisoning das CVEs de ONNX
      // (CVE-2025-51480 / CVE-2026-28500 / CVE-2026-34447). O modelo vem SÓ do
      // hub oficial + cache; o `nrv find` roda de cwd arbitrário, então um repo
      // hostil não pode plantar um ./models/<nome> malicioso e ser carregado.
