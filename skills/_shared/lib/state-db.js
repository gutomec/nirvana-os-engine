...
// ── memory_records (temporal, supersede-never-delete) ───────────────
function recordMemory(handle, { business_slug, statement, source, supersedes } = {}) {
  if (!handle?.available) return null;
  const now = new Date().toISOString();
  const r = handle.db.run(
    'INSERT INTO memory_records (business_slug, statement, source, valid_from, recorded_at) VALUES (?, ?, ?, ?, ?)',
    [String(business_slug || ''), String(statement || ''), source || null, now, now],
  );
  const newId = Number(r.lastInsertRowid);
  if (supersedes) {
    handle.db.run('UPDATE memory_records SET superseded_by = ? WHERE id = ? AND superseded_by IS NULL', [newId, Number(supersedes)]);
  }
  return newId;
}

function supersedeMemory(handle, id, byId) {
  if (!handle?.available) return false;
  handle.db.run('UPDATE memory_records SET superseded_by = ? WHERE id = ?', [byId == null ? null : Number(byId), Number(id)]);
  return true;
}

function activeMemories(handle, businessSlug, limit = 20) {
  if (!handle?.available) return [];
  return handle.db.query(
    'SELECT id, statement, source, valid_from FROM memory_records WHERE business_slug = ? AND superseded_by IS NULL ORDER BY id DESC LIMIT ?',
  ).all(String(businessSlug || ''), limit);
}

function listMemoryHistory(handle, businessSlug, limit = 100) {
  if (!handle?.available) return [];
  return handle.db.query(
    'SELECT id, statement, source, valid_from, superseded_by, recorded_at FROM memory_records WHERE business_slug = ? ORDER BY id DESC LIMIT ?',
  ).all(String(businessSlug || ''), limit);
}
