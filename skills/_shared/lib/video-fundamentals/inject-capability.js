#!/usr/bin/env node
/**
 * inject-capability.js — injects the canonical video capability (media.video.compose)
 * into the squad.yaml of every video-producing squad, and drops a tasks/video-compose.md
 * pointing at the shared fundamentals (so the invoke.ref resolves → zero warnings).
 * Idempotent: re-running is a no-op.
 *
 * Usage:
 *   node inject-capability.js                 # injects into the canonical list
 *   node inject-capability.js --list          # shows the list
 *   node inject-capability.js --slug <slug>   # injects into a single squad
 *   node inject-capability.js --dry-run       # reports what would change
 */
...
// The video generators (squads). Businesses are retrofitted at the employee, not here.
...
// Task doc dropped into each squad so the invoke.ref resolves. Points at the
// shared lib — does not duplicate method, only instructs applying it.
...
function injectInto(slug, dryRun) {
  ...
  // Self-correcting: capability present but with a divergent invoke (e.g. old ref) → reconcile.
