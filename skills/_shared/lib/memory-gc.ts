...
export async function runGc(store: MemoryStore, opts: GcOpts = {}): Promise<GcReport> {
  ...
  // Load before reading: `_allEntries()` returns the in-memory index, and a
  // freshly constructed store has an empty one. Without this the GC reported
  // "0 -> 0 entries" on a store full of memories and collected nothing — which
  // nobody noticed because nothing could reach the GC in the first place.
  await store.load();
