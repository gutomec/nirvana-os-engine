 ...
 *   2. <projectRoot>/.nirvana/outputs   (default — keeps run artifacts inside the project)
 *   3. <HOME>/.nirvana/outputs          (fallback when not inside any project)
 *
...
export function outputsDir(scope: ResolvedScope): string {
  ...
  if (scope.projectRoot) return path.join(scope.projectRoot, ".nirvana", "outputs");
