  ...
  if (mode === "global" && !projectRoot && !process.env.NIRVANA_PROJECT_ROOT && !process.env.NIRVANA_SCOPE_QUIET) {
    ...
    if (inSkillsDir) {
      process.stderr.write(
        ...
        "  path (bun ~/.nirvana/skills/.../scripts/<loader>.ts), or export\n" +
 ...
 *   2. <projectRoot>/outputs            (default — VISIBLE: deliverables belong to the user)
 *   3. <HOME>/.nirvana/outputs          (hidden fallback when outside a project)
 *
