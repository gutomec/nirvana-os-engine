// kinds/squad.ts — the squad module of the admission gate, trivial for now.
//
...
// kinds. The full squad catalog lands in PR1 of the program plan.
...
export async function check(ctx: CheckContext): Promise<Finding[]> {
  const out: Finding[] = [];
  const file = path.join(ctx.dir, "squad.yaml");
  try {
    const doc = parseYaml(fs.readFileSync(file, "utf8"));
    if (!doc || typeof doc !== "object" || Array.isArray(doc)) out.push(mk("manifest_parse", "squad.yaml is not a YAML mapping", typeof doc));
  } catch (e: any) {
    out.push(mk("manifest_parse", "squad.yaml does not parse", String(e?.message ?? e).split("\n")[0]));
  }
  out.push(...surfaceFindings(ctx.dir, "squad", mk));
  return out;
}
...
export const squadModule: KindModule = {
  ...
  criteria,
  check,
  fixers: { surface_regen: surfaceRegenFixer("squad") },
