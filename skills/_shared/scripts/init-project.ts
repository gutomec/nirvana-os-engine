...
/**
 * Copia uma árvore SEM seguir symlinks.
 *
 * lstatSync (não statSync): a árvore de skills tem um symlink `node_modules`
 * dentro de CADA skill apontando para ~/.nirvana/node_modules. Seguindo eles,
 * a cópia materializava centenas de MB por skill e entrava em recursão infinita
 * nos ciclos de `node_modules/.bin/*` (ELOOP / stack overflow), deixando o
 * projeto pela metade. `node_modules` fica de fora em QUALQUER profundidade
 * (não só no topo): a árvore embarcada se liga com `bun install` no primeiro
 * uso. Os demais symlinks são recriados como symlinks, nunca expandidos.
 */
...
function main() {
  ...
  if (!withSkills) {
    ...
    if (!fs.existsSync(canonical)) {
      ...
      for (const e of fs.readdirSync(HOME_SKILLS)) {
        ...
        try {
          ...
          // Fail-closed: engolir aqui deixava um projeto pela metade saindo 0.
    ...
    // O pai (<target>/.agents) NÃO vem do skeleton — sem criá-lo o symlinkSync
    // falha com ENOENT e cai no fallback de cópia à toa (era o gatilho real do
    // init quebrado com --with-skills).
    ensureDir(path.dirname(canonical));
    let needCreate = true;
