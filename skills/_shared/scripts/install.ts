...
function wireLocalBinOnPath(dry: boolean): string[] {
  ...
  if (process.platform === "win32") {
    // user-scoped, idempotent: read User PATH, prepend if missing, write back.
    const ps =
      "$d=[Environment]::GetEnvironmentVariable('PATH','User'); " +
      "$b=Join-Path $env:USERPROFILE '.local\\bin'; " +
      "if (($d -split ';') -notcontains $b) { " +
      "[Environment]::SetEnvironmentVariable('PATH', ($b + ';' + $d), 'User'); " +
    ...
    try {
      ...
      if (r.status === 0 && /added/.test(r.stdout || "")) notes.push(`added ${localBin} to the user PATH (Windows) — abra um NOVO terminal.`);
      else if (/present/.test(r.stdout || "")) { /* already there */ }
      else notes.push(`não consegui ajustar o PATH automaticamente — adicione "%USERPROFILE%\\.local\\bin" ao PATH do usuário.`);
    } catch { notes.push(`adicione "%USERPROFILE%\\.local\\bin" ao PATH do usuário (Windows).`); }
    return notes;
  }
