#!/usr/bin/env bun
/**
 * Conjunto de avaliação para o roteamento de mind-clones (3 eixos).
 *
 *   1. auto-recuperação — o one_liner de cada clone com bloco deve recuperá-lo
 *   2. necessidade — as 47 consultas escritas antes de os blocos existirem
 *   3. andaime — pares idênticos com e sem o verbo de intenção; o top-3 tem
 *      que ser o MESMO nos dois, senão o andaime está decidindo
 *
 * Rode antes e depois de qualquer mudança no tokenizer/índice e compare.
 * Uso: bun eval-clone-routing.ts — ou importe runEval(); é o que
 * tests/clone-routing-eval.test.ts faz para travar os marcos d'água no CI.
 */
...
const NEED: Array<[string, string]> = [
  ...
  // vizinhos que NÃO podem ser deslocados
