...
function syncKind(kind: string, srcRoot: string, dstRoot: string, available: string[], old: Record<string, string>): SyncRes {
  ...
  for (const slug of available) {
    ...
    // Colisão: existe em disco mas o pack nunca o possuiu (fora do manifesto) —
    // é criação do usuário com o mesmo slug. O pack vence (é a fonte da verdade)
    // e não há backup; avisar só torna a perda visível, não a evita.
      ...
      if (prev !== h) {
        ...
        // ANTES do mirror: única janela em que instalado e recebido coexistem.
...
// Quebras de contrato vêm ANTES das colisões: um "~ updated" não distingue
// melhoria de prosa de uma capability que sumiu, e é a segunda que faz o projeto
// do usuário parar de funcionar sem ninguém perceber.
reportBreaks([sq, bz, cl].flatMap((r) => r.breaking), DRY, (m) => console.log(m));

// Colisões de slug em bloco próprio: é a única linha que significa perda de
// trabalho do usuário, e some no meio da contagem se não for destacada.
...
if (!DRY) {
  ...
  // Reindexa: sem isto o conteúdo fica no disco e o orquestrador não o enxerga
  // — a falha mais cara possível, porque tudo PARECE instalado.
  //
  // Antes só tentava `~/.local/bin/nrv` e pulava EM SILÊNCIO se o binário
  // estivesse em outro lugar. Agora cai para o indexador do próprio engine, que
  // é onde ele sempre está: quem chega aqui já tem o engine, porque este script
  // vive dentro dele.
