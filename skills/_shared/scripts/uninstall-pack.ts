...
for (const kind of ["squads", "businesses", "mind-clones"] as const) {
  ...
  for (const comp of Object.keys(comps)) {
    ...
    // businesses/memory/ guarda baseline do pack + projects/ do usuário. Preserva
    // SÓ projects/ (= runstate do install-content); o baseline do pack sai.
    ...
    // Run-state realmente preservado (lido do disco após o prune; previsto no --dry).
    const survivors = DRY
