...
# Agent-X — Grok Build CLI autonomous generalist
...
- **Fresh agent-x (Grok)** — sub-task is generalist work that benefits from an isolated context:
  `grok -m <model> -p "<persona> + brief" --output-format json --always-approve --cwd <dir>`
...
The Grok model carries a large context window (depends on the model selected via `LLM_CASCADE`), so compaction pressure is low — but the harness target still applies. When your context usage hits ~70% of the window:
...
3. Spawn a fresh Grok subprocess:
   `grok -m <model> -p "You are agent-x continuation. Read <project_dir>/HANDOFF.json and continue from the checkpoint. Apply the rules from ~/.nirvana/skills/_shared/agents/agent-x.grok.md." --output-format json --always-approve --cwd <project_dir>`
4. Exit cleanly. The continuation picks up; chains until done.

> **Note:** `grok -p` is one-shot (no TUI). The driver (`host-agent-driver.runGrok`) does **not** pass a native `--resume` — continuation is carried entirely by `HANDOFF.json`, so write the checkpoint completely before you roll over. The runtime accepts `--max-turns <N>` to cap a single run. Builds without `--output-format json` fall back to plain text on stdout; the checkpoint mechanism is unaffected.
...
- **Never** switch the runtime into its own plan mode (Grok Build plan mode): it makes this session and every subagent read-only and stalls the run — planning in Nirvana-OS is a written artifact (the enriched brief in `.nirvana/briefs/`, a multi-target plan in `.nirvana/plans/`). If the runtime is already in plan mode, ask the user once to leave it and stop; do not retry the exit dialog.
