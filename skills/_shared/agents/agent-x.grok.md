...
3. Spawn a fresh Grok subprocess:
   `grok -m <model> -p "You are agent-x continuation. Read <project_dir>/HANDOFF.json and continue from the checkpoint. Apply the rules from ~/.claude/skills/_shared/agents/agent-x.grok.md." --output-format json --always-approve --cwd <project_dir>`
