...
- **Fresh agent-x (Grok)** — sub-task is generalist work that benefits from an isolated context:
  `grok -m <model> -p "<persona> + brief" --output-format json --yolo --cwd <dir>`
...
3. Spawn a fresh Grok subprocess:
   `grok -m <model> -p "You are agent-x continuation. Read <project_dir>/HANDOFF.json and continue from the checkpoint. Apply the rules from ~/.claude/skills/_shared/agents/agent-x.grok.md." --output-format json --yolo --cwd <project_dir>`
