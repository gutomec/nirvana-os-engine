...
3. Spawn a fresh subagent:
   `Agent({subagent_type: "general-purpose", prompt: "You are agent-x continuation. Read <project_dir>/HANDOFF.json and continue from the checkpoint. Apply the same rules from ~/.nirvana/skills/_shared/agents/agent-x.claude-code.md."})`
