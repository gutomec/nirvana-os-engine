...
3. Spawn a fresh Kimi subprocess:
   `kimi -m <model> -p "You are agent-x continuation. Read <project_dir>/HANDOFF.json and continue from the checkpoint. Apply the rules from ~/.nirvana/skills/_shared/agents/agent-x.kimi.md." --output-format stream-json`
...
- **Never** switch the runtime into its own plan mode (Kimi Code plan mode): it makes this session and every subagent read-only and stalls the run — planning in Nirvana-OS is a written artifact (the enriched brief in `.nirvana/briefs/`, a multi-target plan in `.nirvana/plans/`). If the runtime is already in plan mode, ask the user once to leave it and stop; do not retry the exit dialog.
