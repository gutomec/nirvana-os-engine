...
3. Spawn a fresh Codex subprocess:
   `codex exec --json --skip-git-repo-check -C <project_dir> <<< "You are agent-x continuation. Read <project_dir>/HANDOFF.json and continue from the checkpoint. Apply the rules from ~/.claude/skills/_shared/agents/agent-x.codex.md."`
